...
| `StudySearch` | `passage`, `purpose` (`research` or `priorArt`), `tool`, `query`, `servesObjective`, `claims?`, `resultIds`, `error?`, `throttled?` (it failed from a throttle), `retry?` (the second try of a throttled prior-art search), `skipped?` (`maxSearches`), `runId` |
...
| `citableUrl(url)`, `normalizeUrl(url)` | `string \| undefined` | The URL a search result is cited by (tracking parameters and fragment removed), and the one it is known by for its id and duplicates (also host in lower case, no trailing slash); `undefined` for anything but http(s) |
...
`web_search` asks its providers in order; a provider that throws hands over to the next one. One that throws `SearchThrottledError` (`retryAfterMs?`, the wait it asked for) or answers HTTP 429 gets one more try after that wait or `throttleWaitMs` (10 s), when the call's deadline leaves room, and none when it asked for more than 30 s; still throttled (or failing three times in a row), it is skipped for `circuitBreaker.cooldownMs` (2 minutes), or as long as it asked if that is longer. Requests to a host go one at a time, each `minIntervalMs` after the previous one ended, for the whole process (DuckDuckGo: 4 s by default). The web tools throw `WebRequestRefusedError` for what they refuse on purpose (`reason`: `private-address`, `scheme`, `downgrade`, `redirects`, `robots`, `content-type`, `too-large`, `unreadable`, `pacing`), `WebHttpError` for a non-2xx answer (`status`), `WebTimeoutError` (a request, the call's deadline, or the time budget of an extraction), `WebConfigurationError` for a missing setup (`unpdf`, a GitHub token) and `SearchUnavailableError` when no provider answered (`failures`; `throttled` when they all were, `retryAfterMs?` when a skipped one will be tried again). `retry` never retries a refusal, a missing setup, a search no provider answered or a throttle (HTTP 429, `SearchThrottledError`).
