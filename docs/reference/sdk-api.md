...
| `retry` | `Partial<RetryPolicy> \| false` | LLM retry policy, per provider, before fallback. Also the default retries of the Jev client (`jev.maxRetries` and `jev.retryBaseDelayMs` override it). Applied to an injected `llmProvider` only when set explicitly, and never to a `FallbackProvider` given as `llmProvider` or to its providers |
