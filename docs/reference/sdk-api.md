...
| `providerConfig` | `{ openai?, anthropic? }` | Per-provider `apiKey`, `defaultModel` and `baseURL` (a compatible endpoint, such as Azure OpenAI or a local model server, or a proxy) |
