...
| `lines` | One per model, model asked for and source: calls, tokens of the calls that reported them, `unmeteredCalls` when there are any, `totalOnlyTokens` for the totals some of them reported alone, `costUsd` when the model has a price and some of the line's calls reported their tokens; `model` is `(unknown)` for a call that recorded no model name |
