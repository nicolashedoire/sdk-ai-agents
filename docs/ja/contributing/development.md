...
- モジュールごとのユニットテスト
- Vitest を使用
- 実際の有料サービスを呼び出すテストはありません。新しいモジュールのテストスイートはモジュールのモックを使いません。SDK のポートは `src/__tests__/support/` のテストダブルが実装し、HTTP アダプターはローカルのサーバーに対して動きます
- 古いテストスイート（SDK のエントリーポイント、OpenAI と Anthropic のプロバイダー、推論エンジン、フォールバック、リグレッションテストの実行）は、今もベンダーのライブラリ `openai` と `@anthropic-ai/sdk` を `vi.mock` で置き換え、イベントストアなど一部の依存関係を `vi.fn()` で置き換えています

### 統合テスト {#integration-tests}

- 一連のワークフロー全体を通す統合テスト
- LLM プロバイダーはモック化したベンダーのライブラリ（`vi.mock`）で置き換え、実際の API を呼び出すテストはありません
- さまざまなイベントストアでのテスト
...
1. `src/__tests__/support/` のテストダブル、または古いテストスイートのモックが、置き換えているインターフェースと今も一致しているかを確認する
2. 依存関係が最新かを確認する
