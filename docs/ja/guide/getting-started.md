...
必要な環境：**Node.js 20 以上**、TypeScript 5 以上、**v3 系の zod 3.25.28 以降**。zod 4 のスキーマにはまだ対応していません。このパッケージは **ESM 専用**です。`import` で読み込んでください。CommonJS のコードからは動的な `import()` で読み込めます。MCP コネクターを使うには、さらに公式の MCP SDK が必要です。
