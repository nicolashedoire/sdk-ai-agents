...
サービスに制限されたために失敗した先行技術の検索（SDK の Web ツールと同じように、そのツールのエラーが `throttled` を示すもの）は、それで失われるわけではありません。ほかの先行技術の検索の後、サービスが求めた待ち時間（最短 5 秒）を待ってから、もう一度だけ試されます。ただし、実行にまだ 1 分と検索の残りがあるときに限ります。サービスが 150 秒を超える待ち時間、または実行の残り時間より長い待ち時間を求めた検索は、求められたよりも早く再び試されることはありません。2 回目の試行は、それ自体で 1 つの検索として記録されます（`retry: true`）。ほかの理由で失敗した検索は再び試されず、2 回目の試行も失敗した主張は、上に述べたとおり確認すべきもののままです。
