...
| `npm install github:nicolashedoire/sdk-ai-agents …` | この SDK（まだ npm にないので GitHub から。インストール時に自動でビルドされる）、zod（引数を記述するため）、公式の MCP SDK（1.x 系の 1.30 以降。この SDK がテストされているバージョン）をインストールする。 |
