...
すべての型付き決定は、そのコンテキスト、質問、回答、トークン使用量とともに `decision.evaluated` イベントとして書き込まれます。書き込み先は、あなたが渡した `runId`、または専用の `decision_*` ストリームです。Jev の価格は **入力 100 万トークンあたり 0.042 ドル、出力は無料**（2026-09-23 時点のドキュメントによる）なので、`sdk.getRunCost(runId)` は何も設定しなくてもこれを含めます。各決定は期間ごとの予算にも数えられます（指定した `agentId` の予算と、エージェントを指定しない上限に）。予算が決定を拒否することはありません（[API コスト](./costs#budgets) を参照）。

質問と合わない回答（選択肢にない選択、欠けている回答や型の違う回答）も、課金されたことに変わりはありません。そのためこうした回答も、`error` と空の `answers` とともに記録され、その後でエラーが投げられます。バックエンドがトークン数を報告しない場合、イベントには `usage` がなく、呼び出しのコストは 0 ドルではなく不明として報告されます。[API コスト](./costs#unknown-costs) を参照してください。
