...
コストは一つの側面にすぎません。ポリシーを使えば、エージェント、ツール、期間ごとに、**ステップ数、トークン数、ツール呼び出しの回数** に上限を設けることもできます。[ガバナンス付きエージェント](./governed-agents) を参照してください。認知エージェントには、独自の制限（`maxSteps`、`maxToolCalls`、`timeoutMs`）があります。 `maxCost` を指定した `budgetLimit` は、上記の料金に基づき、エージェントのモデル呼び出しが期間ごとに使う金額（米ドル）に上限を設けます。料金のないモデルがあると上限を確認できないため、ツール呼び出しは拒否されます。
