...
features:
  ...
  - icon: 🧭
    title: 針路を外れない研究者
    details: 理解し、今日の手段で設計し直してほしい対象を与えてください。あなたの情報源を検索し、確立された事実を仮説や新規性と区別し、どれを選ぶかを決める実験を提案します。凍結された憲章と監視役が、研究を目的から外れさせません。
    link: /ja/guide/studies
    linkText: 研究
  - icon: 🎯
    title: Jev による型付き決定
...
| 目的から外れない調査 | 指示が積み重なるにつれて逸脱していくチャット | 研究：凍結された憲章、呼び出しのたびに組み立て直されるプロンプト、監視役、情報源と照らし合わせてチェックされる主張 |
