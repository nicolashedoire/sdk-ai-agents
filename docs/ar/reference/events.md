...
| `study.passage_started` | `passage`، و`number` (من 1 إلى 7)، و`amendments` (أرقام التعديلات المقبولة السارية)، و`reopenedBy?` و`focus?` و`reason?` حين تكون مرحلة لاحقة قد أعادت فتحها، و`redo?` و`resumed?` للإعادة المستحقّة لها التي يُجريها تشغيلٌ مستأنَف، و`outdated?` حين تُشغَّل من جديد من أجل عناصر حكم عليها الحارس متأخرًا |
| `study.passage_completed` | `passage`، و`attempts` (2 حين أعادها الحارس)، و`keptAttempt?` (1 حين كانت المحاولة الأولى أفضل من الإعادة فاحتُفظ بها)، و`discarded?` (`attempt`، `items`: الإعادة التي استُبعدت عندئذٍ)، و`items` (كلٌّ منها مع `collection`، و`id`، و`statement`، و`status`، و`sources`، و`servesObjective`، وحقول الادعاء الأخرى، وحقوله الخاصة)، و`reopenedBy?`، و`reopen?` (`passage`، `focus`، `reason`: المرحلة السابقة التي تطلب إعادة فتحها؛ ولا تكون مكتملة إلى أن تُشغَّل من جديد)، و`resumed?` (استأنفها تشغيلٌ لإتمامها فقط)، و`duplicates?` (أحكام أُعطيت من جديد على خيوط سبق الحكم عليها: أُسقطت، وليست انحرافًا) |
| `study.search` | `passage`، و`purpose` (`research`، أو `priorArt` للأعمال السابقة لادعاءات الجِدّة)، و`tool`، و`query`، و`servesObjective`، و`claims?` (ادعاءات الجِدّة التي يبحث عنها)، و`resultIds`، و`results` (`id`، `title`، `locator`، `date?`)، و`error?` (فشل البحث)، و`throttled?` (فشل بسبب تقييد المعدّل)، و`retry?` (المحاولة الثانية لبحث أعمال سابقة قُيِّد معدّله)، و`skipped?` (`maxSearches`: لم يُجرَ) |
| `study.model_called` | `purpose` (`passage`، `queries`، `check`، `priorArtQueries`، `priorArtCheck`، `amendment`)، و`passage?`، و`model?`، و`requestedModel?`، و`usage` (`promptTokens`، `completionTokens`، `calls`، `unmeteredCalls?`، `unmeteredTokens?`: حدث واحد للاستدعاء وإصلاحه)، و`failed?` (سبب تعذّر استخدام الرد)، و`usedAttempt?` (1 حين تعذّر استخدام الإصلاح فاستُخدم الرد الأول، مقروءًا بتساهل) — يُحتسَب في التكاليف وفي الميزانيات لكل فترة |
