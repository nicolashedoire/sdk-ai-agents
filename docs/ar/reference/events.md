...
| `cognition.thought` | `step`، و`operation`، و`patch`، و`issues`، و`failed`، و`ignoredFields?`، و`model?`، و`requestedModel?`، و`usage?` (`promptTokens`، `completionTokens`، `calls`) |
| `cognition.operation_failed` | `step`، و`operation`، و`error`، و`recovery?` |
...
| `decision.evaluated` | `client`، و`purpose` (`operation_selection`، `hypothesis_assessment`، `direct`)، و`model`، و`state`، و`questions`، و`answers`، و`usage`، و`step?` |
