...
| `includeStreamUsage` | على واجهة OpenAI البرمجية نفسها | `true`: يُطلَب من الإجابة المبثوثة استهلاكها (`stream_options`)، فتُحتسَب كلفتها؛ `false`: لا يُطلَب. وهو مفعّل افتراضيًا على `https://api.openai.com/v1` وعلى العناوين الإقليمية مثل `https://eu.api.openai.com/v1` (من `baseURL` أو `OPENAI_BASE_URL`)، إذ قد يرفض الخادم المتوافق هذا الحقل (فيُرسَل الطلب عندئذٍ من جديد دونه، ولا يُعاد إلى واجهة OpenAI البرمجية نفسها إلا إذا عُيّن `includeStreamUsage`) أو يتجاهله، والاستدعاء المبثوث الذي لا يأتي معه استهلاكه يُعَدّ غير مُقاس. ومع ميزانية للكلفة على خادم متوافق يُبلغ عن الاستهلاك (كما تفعل واجهة v1 من Azure OpenAI)، عيّن `true` |
...
| `lines` | سطر لكل نموذج، ونموذج مطلوب، ومصدر: الاستدعاءات، ورموز الاستدعاءات التي أبلغت عنها، و`unmeteredCalls` إن وُجدت، و`totalOnlyTokens` للمجاميع التي أبلغ عنها بعضها وحدها، و`costUsd` حين يكون للنموذج سعر وأبلغ بعض استدعاءات السطر عن رموزها؛ وقيمة `model` هي `(unknown)` لاستدعاء لم يسجّل اسم نموذج |
