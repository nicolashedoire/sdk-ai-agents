...
| `StudySearch` | `passage`، و`purpose` (`research` أو `priorArt`)، و`tool`، و`query`، و`servesObjective`، و`claims?`، و`resultIds`، و`error?`، و`throttled?` (فشل بسبب تقييد المعدّل)، و`retry?` (المحاولة الثانية لبحث أعمال سابقة قُيِّد معدّله)، و`skipped?` (`maxSearches`)، و`runId` |
...
| `citableUrl(url)`، `normalizeUrl(url)` | `string \| undefined` | عنوان URL الذي يُستشهَد به لنتيجة بحث (بعد إزالة معاملات التتبّع والجزء الذي يلي `#`)، والعنوان الذي تُعرَف به في معرّفها وفي كشف التكرارات (مع اسم المضيف بأحرف صغيرة أيضًا، ودون الشرطة المائلة الختامية)؛ و`undefined` لأي شيء غير http(s) |
...
يسأل `web_search` مزوّديه بالترتيب؛ والمزوّد الذي يرمي خطأً يسلّم إلى التالي. والذي يرمي `SearchThrottledError` (`retryAfterMs?`، الانتظار الذي طلبه) أو يجيب بـ HTTP 429 يحصل على محاولة أخرى بعد ذلك الانتظار أو `throttleWaitMs` (10 ثوانٍ)، حين تتّسع المهلة النهائية للاستدعاء لذلك، ولا يحصل على أي محاولة حين طلب أكثر من 30 ثانية؛ وإن بقي مقيَّدًا (أو فشل ثلاث مرات متتالية)، يُتخطّى طوال `circuitBreaker.cooldownMs` (دقيقتان)، أو طوال ما طلبه إن كان ذلك أطول. وتذهب الطلبات إلى المضيف الواحد واحدًا تلو الآخر، كلٌّ منها بعد `minIntervalMs` من انتهاء السابق، على مستوى العملية كلها (DuckDuckGo: 4 ثوانٍ افتراضيًا). وترمي أدوات الويب `WebRequestRefusedError` لما ترفضه عن قصد (`reason`: `private-address`، `scheme`، `downgrade`، `redirects`، `robots`، `content-type`، `too-large`، `unreadable`، `pacing`)، و`WebHttpError` لإجابة ليست 2xx (`status`)، و`WebTimeoutError` (لطلب، أو للمهلة النهائية للاستدعاء، أو لميزانية الوقت المخصّصة للاستخراج)، و`WebConfigurationError` لإعداد ناقص (`unpdf`، أو رمز GitHub)، و`SearchUnavailableError` حين لا يجيب أي مزوّد (`failures`؛ و`throttled` حين كانت كلها مقيَّدة، و`retryAfterMs?` حين سيُجرَّب مزوّد متخطّى من جديد). ولا يعيد `retry` أبدًا محاولة رفضٍ، ولا إعداد ناقص، ولا بحث لم يُجب عنه أي مزوّد، ولا تقييد للمعدّل (HTTP 429، `SearchThrottledError`).
