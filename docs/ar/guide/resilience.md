...
المزوّد الذي تحقنه بـ `llmProvider` يُستخدَم كما هو ما لم تعيّن `retry` صراحةً، ولا يُغلَّف `FallbackProvider` أبدًا حتى تبقى تحويلاته إلى البديل مرئية في الأثر. ولا تُغلَّف مزوّداته كذلك، فلا ينطبق عليها `retry`: لإعادة محاولة أحدها قبل التحويل إلى البديل، غلّفه في `RetryingLLMProvider` وأعطِ عميله `maxRetries: 0`.
