...
- **حدث واحد في كل مرة، والتشغيل لا ينتظر أبدًا.** حين يعيد مستمِعك وعدًا (promise)، ينتظر حدثه التالي إلى أن يستقرّ ذلك الوعد، فلا يستطيع مستمِع غير متزامن أن يغيّر ترتيب الأحداث. ويمضي التشغيل في الأثناء: المستمِع البطيء يتأخّر، ولا يبطّئ الوكيل. ويُستدعى المستمِع المتزامن قبل أن يعود `append` الخاص بالحدث: فاجعله سريعًا.
- **الاستدعاء ينتظر المستمِع، إلى أن يُقاطَع التشغيل.** بعد انتهاء التشغيل، تنتظر `run()` و`think()` و`replay()` و`executeTool()` إلى أن يفرغ `onEvent` الخاص بها من كل حدث، فحين تعود تكون قد رأيت كل شيء. وينتهي الانتظار مبكرًا حين يكون التشغيل قد أُوقِف أو أُلغي، أو حين يُلغى `signal` الخاص به (يتخلّى المستدعي)؛ وفي التشغيل المعرفي، تحتسب `limits.timeoutMs` مدة الانتظار أيضًا. عندئذٍ يُلغى اشتراك المستمِع: وتُسقَط الأحداث التي لم يتلقّها بعد. ولا يمكن إلغاء إعادة التشغيل، لذا فهي تنتظر دائمًا. وفيما عدا ذلك، يمنع الوعدُ الذي لا يستقرّ أبدًا الاستدعاءَ من العودة: للعمل الذي تطلقه دون انتظاره، لا تُعِد الوعد (`onEvent: (event) => { void save(event); }`).
- **طابور محدود.** لا ينتظر المستمِعَ المنشغلَ بعدُ بحدث سابق إلا `maxQueued` حدثًا على الأكثر (10 000 افتراضيًا)؛ وبعد ذلك، تُسقَط الأحداث الجديدة بالنسبة إلى هذا المستمِع. ويُبلَّغ عن الإسقاط حين يلحق المستمِع بالركب، بخطأ `LiveEventsDroppedError` يذكر عدد الأحداث التي أُسقِطت.
- **الأخطاء تبقى خارج التشغيل.** المستمِع الذي يرمي خطأً أو يُرفَض وعده يُبلَّغ عنه على الخطأ القياسي (`console.error`) ويظل يتلقّى الأحداث التالية؛ وكذلك يُبلَّغ عن الأحداث المُسقَطة. لمعالجة الأخطاء بنفسك، التقطها داخل المستمِع؛ ولمعالجة الأخطاء والأحداث المُسقَطة على حدّ سواء، أنشئ حزمة SDK مع `eventStore: new ObservedEventStore(store, { onListenerError })`.
...
يطابق المرشِّح `agentId` الحقلَ `metadata.agentId` في كل حدث: بعض الأحداث لا تحمل وكيلًا (`provider.retry`، ونهاية إعادة التشغيل، و`decision.evaluated` الخاص باستدعاء `sdk.decisions` أُجري دون `agentId`)، فصفِّ حسب التشغيل للحصول عليها. ولا تُسلَّم النسخ الاحتياطية المُستعادة. أما تقارير الحوادث الصادرة عن `incidents` فتُسلَّم؛ ومع `MonitoredEventStore` تبنيه بنفسك، ابنِه فوق `ObservedEventStore` (`new MonitoredEventStore(new ObservedEventStore(store), options)`)، وإلا فستُسجَّل تقاريره لكنها لن تُسلَّم مباشرةً. ولوكيل مُجمَّع يدويًا على مخزنك الخاص (`new AgentImpl(…)`)، غلِّف المخزن بـ `ObservedEventStore` لاستخدام `onEvent`؛ أما `createSDK` فيفعل ذلك نيابةً عنك. وخلف أداة وكيل، يعمل الوكيل الذي لم يُغلَّف مخزنه دون مستمِع المستدعي بدل أن يفشل.
...
تُقارَن عمليات التشغيل بحسب ما تعنيه أحداثها — النوع، والترتيب، والأداة، والمعاملات، والنتائج — ولا تُقارَن أبدًا بمعرّفات الأحداث، الجديدة في كل تشغيل؛ وتُستبعَد الأوقات وأعداد الرموز (tokens) كذلك. التشغيل الذي يفعل الشيء نفسه مرة أخرى ينجح؛ والأداة التي تُستدعى بوسائط أخرى يُبلَّغ عنها حيث وقع الاستدعاء.
