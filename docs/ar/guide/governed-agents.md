...
- مع `onText`، يستخدم المزوّدان المدمجان OpenAI وAnthropic واجهة البث التدريجي (streaming) لدى مورّدهما، وتبقى نتيجة التشغيل وأحداثه كما هي دونه. وكذلك كلفته، إلا مع خادم متوافق مع OpenAI (عبر `baseURL` أو `OPENAI_BASE_URL`): فاستهلاك الإجابة المبثوثة لا يُطلَب إلا على واجهة OpenAI نفسها، بما في ذلك عناوينها الإقليمية، ما لم يُعيَّن `providerConfig.openai.includeStreamUsage`، والاستدعاء الذي لا يأتي معه استهلاكه يُعَدّ غير مُقاس في الميزانيات. ومع ميزانية للكلفة على خادم متوافق يُبلغ عن الاستهلاك (كما تفعل واجهة v1 من Azure OpenAI)، عيّن `includeStreamUsage: true`.
