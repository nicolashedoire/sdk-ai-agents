...
التكلفة جانب واحد فقط؛ إذ يمكن للسياسات أيضًا وضع سقف لـ **الخطوات والرموز واستدعاءات الأدوات** لكل وكيل وأداة وفترة — انظر [الوكلاء الخاضعون للحوكمة](./governed-agents). وللوكلاء المعرفيين حدودهم الخاصة (`maxSteps`، `maxToolCalls`، `timeoutMs`). يحدّ `budgetLimit` مع `maxCost` من الدولارات التي تنفقها استدعاءات النموذج لدى وكيل في كل فترة، وفق الأسعار أعلاه؛ وإذا لم يكن لنموذجٍ سعر، تعذّر فحص الحدّ فتُرفض استدعاءات الأدوات.
