...
- **نص PDF** يُقرأ بالحزمة الاختيارية [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 أو أحدث): `npm install unpdf`. ومن دونها، يقول `web_fetch` ذلك. ويأتي العنوان والتاريخ من المستند. وتُقرأ ملفات PDF واحدًا تلو الآخر داخل العملية، كلٌّ منها في خيط عامل (worker thread). وقبل أن تقرأ مكتبة pdf.js ملف PDF، يفكّ الخيط العامل ضغط تدفّقاته (streams) عبر مرشّحاتها (Flate وLZW وRunLength، بما فيها المتسلسلة، وASCII85 وASCIIHex اللذان يغلّفانها) ويرفض الملف إذا تجاوزت 256 MB في مجموعها: فهذا القياس لا يحتاج شيئًا من بقية العملية، ولذلك يُرفَض ملف PDF صغير يتضخّم إلى غيغابايتات حتى حين يكون الخيط الرئيسي مشغولًا. أما التدفّق الذي لا يستطيع قياسه (المشفَّر، أو ذو المرشّح الذي لا يعرفه، كمرشّح صورة) فيُترَك لمكتبة pdf.js. وكخط دفاع ثانٍ، يُوقَف الخيط العامل إذا كبرت العملية بأكثر من 1 GB أثناء القراءة، أو بعد 20 ثانية، أو حين ينتهي الاستدعاء؛ ويُحتسَب انتظار دوره من المهلة النهائية للاستدعاء.
...
| `retry` | — | إعادة محاولة الاستدعاءات الفاشلة (`{ maxRetries }`): عند تقييد المعدّل، وأخطاء الخادم، وانتهاء المهلة، وإخفاقات الشبكة؛ ولا تكون أبدًا عند رفض، أو إعداد ناقص (`WebConfigurationError`)، أو بحث لم يُجب عنه أي مزوّد (`SearchUnavailableError`). |
...
- **الذواكر المؤقتة وتباعد الطلبات تعيش في الذاكرة**، لكل استدعاء `webTools()`، وتضيع حين تنتهي العملية.
