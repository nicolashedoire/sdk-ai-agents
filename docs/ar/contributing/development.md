...
- تستخدم Vitest
- لا يستدعي أي اختبار خدمة مدفوعة حقيقية. لا تستخدم مجموعات اختبارات الوحدات البرمجية الأحدث محاكاة للوحدات (module mocks): تنفّذ البدائل الاختبارية في `src/__tests__/support/` منافذ حزمة SDK، وتعمل محوّلات HTTP مقابل خوادم محلية
- ما زالت مجموعات الاختبارات الأقدم (نقطة دخول حزمة SDK، ومزوّدا OpenAI وAnthropic، ومحرّك الاستدلال، والبديل الاحتياطي، ومشغّل اختبارات الانحدار) تستبدل مكتبتَي المزوّدين `openai` و`@anthropic-ai/sdk` بـ `vi.mock`، وبعض التبعيات، مثل مخزن الأحداث، بـ `vi.fn()`

### اختبارات التكامل {#integration-tests}

- اختبارات تكامل لسير العمل الكامل
- يُستبدَل مزوّدو النماذج اللغوية بمكتبة مزوّد مُحاكاة (`vi.mock`)؛ ولا يستدعي أي اختبار واجهة API حقيقية
...
1. تحقّق من أن البدائل الاختبارية في `src/__tests__/support/`، أو بدائل المحاكاة (mocks) في مجموعات الاختبارات الأقدم، ما زالت مطابقة للواجهات التي تحلّ محلها
