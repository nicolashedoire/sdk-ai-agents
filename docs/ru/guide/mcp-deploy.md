...
## Уведомления о ходе выполнения {#progress-notifications}

Клиент может попросить сообщать ему, как идёт вызов: он отправляет `progressToken` вместе с вызовом (официальный TypeScript SDK делает это, когда вы передаёте `onprogress`). Тогда сервер отправляет `notifications/progress` на каждое событие вызова, а также запуска агента, который начинает `cognitiveAgentTool` или `governedAgentTool`, с `progress`, который каждый раз увеличивается на единицу, и коротким `message`:
...
Сообщения называют шаги, инструменты и когнитивные операции: инструмент, который выбрала модель, и каждый инструмент, который вызывает агент, включая инструменты агента, которые сервер не открывает. Инструмент, который запускает [исследование](./studies) с `onEvent` своего контекста, описывается по этапам (`study started`, `passage changes started`, `search in changes`, `report ready`), но никогда — по запросам. Сообщения никогда не содержат аргументов, результатов или текстов ошибок. Поля `total` нет: никто заранее не знает, сколько шагов займёт запуск. Каждое уведомление отправляется до результата, никогда после него. По Streamable HTTP они передаются в потоке ответа (SSE); транспорт, созданный с `enableJsonResponse: true`, отвечает обычным JSON и отбрасывает их. Клиент, который не отправляет `progressToken`, их не получает.

Отслеживается только запуск агента, который начинает инструмент, на один уровень: запуски, которые этот агент начинает через собственные инструменты-агенты, не отслеживаются, как и агент, собранный вручную на хранилище без событий в реальном времени. Ничего не объединяется: каждое событие — это отдельное уведомление, и долгий когнитивный запуск может отправить их сотни.

Что меняют уведомления о ходе выполнения, а что нет:

- **Дольше ждут только клиенты, которые сбрасывают свой тайм-аут при уведомлениях о ходе выполнения.** С TypeScript SDK: `client.callTool(params, undefined, { onprogress, resetTimeoutOnProgress: true, maxTotalTimeout })`. Клиент, который показывает ход выполнения, но сохраняет фиксированный тайм-аут, сдаётся в тот же момент, что и раньше. Проверьте, как ведёт себя ваше приложение, прежде чем на это рассчитывать.
- **С таким клиентом важна самая долгая пауза**, а не длительность вызова: один вызов модели, один медленный инструмент или одобрение. Пока инструмент работает или одобрение ждёт решения, ничего не отправляется, поэтому 50 с `approvalTimeoutMs` по-прежнему действуют, и каждый отдельный вызов модели или инструмента должен укладываться в тайм-аут клиента.
- **Тогда агенты могут работать дольше**: `limits.timeoutMs` когнитивного агента может превышать тайм-аут клиента, поскольку каждый шаг отправляет уведомления. Для остальных клиентов сохраняйте небольшие лимиты из [рецепта агента](./mcp-recipes#an-agent-your-reasoning-twin).
...
| Вызовы завершаются по тайм-ауту | Инструмент медленный (часто это агент) | Уменьшите `limits` агента; увеличьте тайм-аут клиента (Claude Code: `MCP_TOOL_TIMEOUT`); или используйте клиент, который сбрасывает свой тайм-аут при [уведомлениях о ходе выполнения](#progress-notifications). |
