...
В запуске, который передаёт свой текст потоком (`onText`), вызов модели может дать сбой после того, как часть ответа уже пришла. Ошибка, которую поставщик присылает посреди потока, приравнивается к соответствующей ошибке HTTP: `server_error` (500) у OpenAI и `rate_limit_error` (429), `api_error` (500), `timeout_error` (504) и `overloaded_error` (529) у Anthropic повторяются, другие типы — нет. Соединение, оборвавшееся посреди потока, поток, который закончился до того, как его ответ стал полным, и поток, который ничего не присылает в течение тайм-аута клиента (по умолчанию 10 минут; `timeout` в `providerConfig`, в `config` резервного провайдера или в параметрах `OpenAIProvider` и `AnthropicProvider`), считаются сбоями соединения. Перед повторной попыткой или перед переходом к резервному провайдеру `onTextRestart` сообщает вызывающей стороне, что текст, переданный неудавшейся попыткой, нужно отбросить: следующая попытка напишет ответ заново (см. [Потоковая передача ответа](./governed-agents#_7-streaming-the-answer)).
