...
Резервному провайдеру другого поставщика нужен собственный ключ — в его `config` или в `providerConfig`: ключ основного провайдера никогда не отправляется другому поставщику. Он получает модель агента, только если обслуживает её, а иначе — свою `defaultModel` (Anthropic отклоняет название модели OpenAI, и наоборот). Событие `intention.generated` указывает, какой провайдер ответил и какой моделью.
...
Провайдер, который вы передаёте через `llmProvider`, используется как есть, если вы явно не задали `retry`, а `FallbackProvider` никогда не оборачивается, чтобы его переключения оставались видимыми в трассе.
