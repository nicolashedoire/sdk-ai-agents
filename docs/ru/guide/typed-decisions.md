...
| `retryBaseDelayMs` | `500` | Первая пауза, удваивается при каждой повторной попытке |
| `maxRetryDelayMs` | `30000` | Верхняя граница каждой паузы, включая `retry-after` |
...
Каждое типизированное решение записывается как событие `decision.evaluated` с контекстом, вопросами, ответами и расходом токенов — в переданный вами `runId` или в отдельный поток `decision_*`. Jev стоит **$0,042 за миллион входных токенов, выходные бесплатно** (согласно документации на 2026-09-23), поэтому `sdk.getRunCost(runId)` учитывает его сразу. Каждое решение также учитывается в бюджетах за период — для указанного в нём `agentId` и в ограничениях без агента, — и бюджет никогда его не отклоняет (см. [Затраты на API](./costs#budgets)).

Ответ, который не соответствует вопросам (выбор, которого нет среди вариантов, отсутствующий ответ или ответ другого типа), всё равно оплачен: он тоже записывается — с `error` и пустым `answers` — до того, как будет выброшена ошибка. Если бэкенд не сообщает число токенов, у события нет `usage`, и стоимость вызова указывается как неизвестная, а не как $0 — см. [Затраты на API](./costs#unknown-costs).
