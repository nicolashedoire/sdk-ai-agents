...
- Провайдер без потоковой передачи (ваш собственный `llmProvider`, если он не читает `onTextDelta`) отдаёт весь текст каждого вызова модели одним куском, когда вызов завершается. Так же поступает OpenAI с моделью, которую он отказывается передавать потоком (организация не прошла для неё верификацию): провайдер запрашивает снова без потоковой передачи и для этой модели её больше не использует. Совместимый сервер, который отклоняет `stream_options`, получает повторный запрос без этого поля (собственный API OpenAI это поле принимает: отклонённый им запрос отправляется снова, только если задан `includeStreamUsage`).
