...
| `executeTool(name, params, { onEvent })` | События вызова, а также запусков агентов, которые начинает его инструмент (`governedAgentTool`, `cognitiveAgentTool`) |
| `sdk.subscribe(listener, { runId?, agentId?, types? })` | Каждое событие каждого запуска, подходящего под фильтр, пока вы не вызовете функцию, которую он возвращает |
...
- **По одному событию за раз, и запуск никогда не ждёт.** Когда ваш слушатель возвращает промис, его следующее событие ждёт, пока этот промис не завершится, поэтому асинхронный слушатель не может перепутать порядок событий. Запуск тем временем продолжается: медленный слушатель отстаёт, но не замедляет агента. `run()`, `think()`, `replay()` и `executeTool()` завершаются, когда их `onEvent` закончил обработку каждого события запуска, поэтому к моменту их возврата вы уже увидели всё. Промис, который никогда не завершается, не даёт им вернуть результат: для работы в режиме «запустил и забыл» не возвращайте промис (`onEvent: (event) => { void save(event); }`). Синхронный слушатель вызывается до того, как `append` события вернёт управление: делайте его быстрым.
- **Ошибки не попадают в запуск.** Если слушатель выбрасывает исключение или его промис отклоняется, ошибка выводится в стандартный поток ошибок (`console.error`), а слушатель по-прежнему получает следующие события. Чтобы обрабатывать ошибки самостоятельно, перехватывайте их в слушателе или создайте SDK с `eventStore: new ObservedEventStore(store, { onListenerError })`.
...
Фильтр `agentId` проверяет `metadata.agentId` каждого события: у некоторых событий агент не указан (`provider.retry`, конец воспроизведения), и чтобы их получить, фильтруйте по запуску. Восстановленные резервные копии не доставляются. Для агента, собранного вручную на вашем собственном хранилище (`new AgentImpl(…)`), оберните хранилище в `ObservedEventStore`, чтобы использовать `onEvent`; `createSDK` делает это за вас.
