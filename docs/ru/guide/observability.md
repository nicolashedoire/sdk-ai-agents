...
Запуски сравниваются по смыслу их событий — тип, порядок, инструмент, параметры, результаты, — а не по id событий, которые у каждого запуска новые; время и число токенов тоже не сравниваются. Запуск, который снова делает то же самое, проходит; инструмент, вызванный с другими аргументами, отмечается там, где произошёл вызов.
