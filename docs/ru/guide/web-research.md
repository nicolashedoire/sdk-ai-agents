...
- Текст **PDF** читается с помощью необязательного пакета [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 или новее): `npm install unpdf`. Без него `web_fetch` так и сообщает. Заголовок и дата берутся из самого документа. PDF читается в рабочем потоке (worker thread), который останавливается, если процесс вырос более чем на 256 МБ, через 20 с или когда вызов завершается: маленький PDF, который раздувается до гигабайт, не может ни заблокировать процесс, ни исчерпать его ресурсы.
...
| `callTimeoutMs` | `60000` | На весь вызов, чего бы он ни ждал: чтение robots.txt, интервалы между запросами, каждое перенаправление, тело ответа и извлечение страницы или PDF. По истечении этого времени всё прерывается, и вызов завершается ошибкой `WebTimeoutError`. |
