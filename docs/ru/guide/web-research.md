...
- Текст **PDF** читается с помощью необязательного пакета [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 или новее): `npm install unpdf`. Без него `web_fetch` так и сообщает. Заголовок и дата берутся из самого документа. PDF читаются в процессе по одному, каждый в рабочем потоке (worker thread). Прежде чем pdf.js прочитает PDF, рабочий поток распаковывает его потоки данных (streams) через их фильтры (Flate, LZW и RunLength, в том числе цепочки из них, а также оборачивающие их ASCII85 и ASCIIHex) и отклоняет PDF, если в сумме выходит больше 256 МБ: этому измерению ничего не нужно от остального процесса, поэтому маленький PDF, который раздувается до гигабайт, отклоняется, даже когда главный поток занят. Поток данных, который он не может измерить (зашифрованный или с незнакомым ему фильтром, например фильтром изображения), передаётся pdf.js. Вторая линия защиты: рабочий поток останавливается, если процесс во время чтения вырос более чем на 1 ГБ, через 20 с или когда вызов завершается; ожидание своей очереди входит в крайний срок вызова.
