...
`seek_information` использует **встроенные движки рассуждения и действий**: LLM выбирает один инструмент для открытого неизвестного, движок действий проверяет, что инструмент был **дан этому агенту**, сверяет вызов с политиками (включая лимиты запуска, см. [Лимиты и политики](#limits-and-policies)), одобрениями и бюджетами, а результат записывается как **наблюдение**, указывающее на своё событие `action.executed`, и затем включается в состояние как факты с `source: "tool"`. Наблюдение сохраняется, даже если его интерпретация не удалась. Отклонённый, заблокированный или сбойный инструмент становится записанной неудачей, и рассуждение продолжается.
...
### Лимиты и политики {#limits-and-policies}

Политики бюджета и тайм-аута, которые действуют для агента, — из его `policies` и глобальные — проверяются **перед каждым шагом**, до любого вызова модели на этом шаге, и снова перед каждым вызовом инструмента. Они видят ход запуска: `maxSteps` считает уже сделанные шаги, `maxTokens` — токены вызовов модели этого запуска (мысли и их исправления, выбор инструментов, типизированные решения, ответы, которые провайдер не смог использовать), `maxDuration` — время с начала запуска. Бюджеты токенов и стоимости за период (`budgetLimit` с `maxTokens` или `maxCost`, без `toolName`) тоже проверяются перед каждым шагом, и в них учитывается каждый вызов модели, который записывает запуск (см. [Затраты на API](./costs#budgets)). Шаг проверяется как намерение типа `continue`: политика, условия которой требуют вызова инструмента (`intention.type` равно `tool_call`), применяется только к вызовам инструментов. Списки разрешённых, пользовательские политики, бюджеты вызовов (`maxToolCalls`) и одобрения касаются только вызовов инструментов, как и правило лимита с действием `require_approval`: шаг никогда не ждёт одобрения. Проверка каждого шага попадает в аудит политик (`sdk.getPolicyAuditTrail`) — для политик, которые могут применяться к шагу.

Запуск завершает первый достигнутый лимит (лимиты вызовов инструментов лишь пропускают вызовы), и два вида лимитов завершают его по-разному:

| Лимит | `limits` агента | Политики |
| --- | --- | --- |
| Шаги | `maxSteps`: последний шаг принимает решение; `completed`, с решением `committed`, `provisional` или `abstain` | `maxSteps`: следующий шаг отклоняется; `failed` |
| Время | `timeoutMs`: запуск прерывается, выполняющийся вызов получает сигнал прерывания; `failed`, `Timeout exceeded (… ms)` | `maxDuration`: проверяется между шагами и перед вызовами инструментов, выполняющийся вызов продолжается; `failed`, `Timeout (… ms) exceeded` |
| Токены, стоимость | — | `maxTokens`, бюджеты за период: следующий шаг отклоняется; `failed` |
| Вызовы инструментов | `maxToolCalls`: `seek_information` больше не предлагается | Отклонённый вызов становится записанной неудачей, и рассуждение продолжается |

Отклонённый шаг записывается как `policy.violated` — с `intention: { type: 'continue' }`, шагом (`step`), причиной (`reason`) и нарушенными политиками (`violatedPolicies`), — затем `run.failed` с причиной политики; у результата `status: 'failed'` и `PolicyViolationError` в `error`. Вызов инструмента, отклонённый лимитом запуска, записывается как `policy.violated` и как неудавшаяся операция; поскольку лимит по-прежнему превышен, следующий шаг отклоняется, и запуск завершается ошибкой. Чтобы запуск заканчивался решением, а не отказом, держите `maxSteps` агента не выше, чем у политики: тогда его последний шаг примет решение раньше, чем политика что-либо отклонит.
...
- `sdk.replay(runId)` повторно выполняет его вызовы инструментов без вызова LLM и воспроизводит итоговый ответ — с тем же ограничением инструментов, что и исходный запуск, поэтому инструмент, в котором агенту было отказано, снова будет отклонён, и с ходом запуска на момент каждого вызова, поэтому вызов, отклонённый лимитом запуска, снова будет отклонён;
