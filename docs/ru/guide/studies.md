...
Запуск проходит семь этапов метода по порядку. Каждый из них производит элементы нескольких видов (свои **коллекции**), и каждый элемент получает идентификатор, который никогда не используется повторно: `O1`, `P2`, `A1`…
...
Результаты, которые возвращают поиски, тоже нумеруются: `S1`, `S2`… Каждый этап получает нужные ему элементы предыдущих этапов в виде компактных записей JSON.
...
Страж (см. [Страж](#the-guardian)) также оценивает каждую архитектуру: возможность, которую он находит лишь более быстрой или более дешёвой, становится `improvement` с `declaredKind: 'capability'`, чтобы было видно, что заявила модель. Проектирование, в котором не осталось ни одной возможности, целиком уходит от цели: это записывается в журнал дрейфа, и проектирование переделывается один раз; если и в переделке нет ни одной возможности, отчёт так и говорит (предупреждение `noCapability`). В отчёте **сначала идут возможности, потом улучшения**.
...
- компонент **никогда не бывает новшеством**: компонент, представленный как новый, получает статус `established`, если его документирует результат, полученный исследованием, и `hypothesis` в остальных случаях, с указанием причины;
- собственный статус архитектуры — это статус её сборки и её возможности. Когда она заявляет новшество, её предшествующие работы ищутся **как сочетание**: исследование ищет работы, которые уже соединяют те же компоненты, чтобы получить ту же возможность, а не каждую часть по отдельности.

Досье показывает путь каждой архитектуры: компоненты (с их статусами) → сборка (с её статусом) → возможность.
...
Каждый прорыв, который вы называете в `analogues`, должен быть разобран. Ответ, в котором какой-то из них забыт, отправляется обратно один раз; если какого-то всё ещё не хватает, отчёт указывает его в `undeconstructedAnalogues` с предупреждением `analoguesNotDeconstructed`. Исследование может добавить другие найденные им прорывы.
...
| --- | --- | --- |
| `established` | Утверждение ссылается хотя бы на один результат, **полученный этим исследованием** из его источников | Утверждение становится `hypothesis`. `declaredStatus` сохраняет статус, который дала модель, а `statusReason` объясняет почему; идентификаторы, на которые оно ссылалось, но которые исследование так и не получило, хранятся отдельно в `unretrievedSources` и ничего не подтверждают |
...
Статус, который исследование не может прочитать, считается `hypothesis` и никогда — более сильным. **Без источников ничего нельзя установить**: каждое утверждение в лучшем случае гипотеза, ни одно новшество нельзя проверить, и отчёт говорит об этом в своём первом предупреждении (`noSources`).

### Предшествующие работы {#prior-art}

После проектирования исследование ищет предшествующие работы для каждого новшества, которое ещё нужно проверить, — как из проектирования, так и из более ранних этапов. Модель выбирает поиски (для архитектуры — сочетание её компонентов и возможность), исследование выполняет их, затем отдельный вызов называет среди результатов наиболее близкую существующую работу и выносит вердикт:

- `novel` или `partlyNovel`: утверждение остаётся новшеством, уже не требующим проверки, со своим `priorArt` (`closest`, `sources`, `verdict`);
- `exists`: идея уже реализована; утверждение становится `hypothesis`, а `statusReason` называет наиболее близкую работу.

Новшество, предшествующие работы которого не удалось найти или оценить — нет источника, бюджет поисков исчерпан, для него не запросили поиск или новшество заявлено после проектирования, — остаётся требующим проверки и сообщает почему. Отчёт их подсчитывает (предупреждение `noveltiesToVerify`).

## Поиск через ваши источники {#research-through-your-sources}

В SDK нет встроенного веб-поиска. Исследование ищет с помощью **инструментов, которые вы ему даёте** как `sources`: это имена инструментов SDK, обычно поисковые инструменты сервера MCP, импортированные через [`connectMcpServer`](./mcp#use-the-tools-of-an-mcp-server-in-your-agents) и определённые через `sdk.defineTool`:
...
- **Нумерованные результаты.** Исследование читает результаты любой формы (список, объект, содержащий такой список, например `results` или `items`, текст JSON, текстовые части MCP, обычный текст), сохраняет заголовок, URL или другой указатель, дату, если она есть, и отрывок и нумерует их один раз на всё исследование: тот же результат, найденный снова, узнаётся по указателю и сохраняет свой идентификатор. Из каждого поиска оно сохраняет `limits.maxResultsPerSearch` результатов (по умолчанию 5).
...
Ваши направления — это примеры для проверки, а не истины: `changes` должен дать каждому из них вердикт — `relevant`, `partlyRelevant` или `notRelevant`, с причинами, — и ответ, в котором одно из них забыто, отправляется обратно один раз. Направление, так и оставшееся без вердикта, указывается в `unverifiedLeads` (предупреждение `leadsNotVerified`). Инструменты, которые исследование находит само, — это `independentLeads`.
...
- задачи этапа;
- компактных записей из предыдущих этапов, которые ему нужны (JSON, а не стенограммы);
- результатов поиска, на которые он может ссылаться.
...
Устав, включая цель, — первое, что читает модель, а цель — последнее.
...
- Когда отклонённых элементов (стражем или схемой) больше определённой доли того, что произвёл этап, — `driftThreshold`, по умолчанию треть, — этап **переделывается один раз**, и ему сообщают, какие элементы были отклонены и почему. Элементы переделки заменяют элементы первой попытки.
- Отчёт сохраняет каждое отклонение (`driftLog`) и подсчитывает отклонения и переделки в `stats`.

### Дополнения {#amendments}

Вы можете добавить инструкцию после создания исследования. Она никогда не проскальзывает незамеченной: страж классифицирует её относительно устава в отдельном запуске.
...
| `conflicts` | Оно противоречит уставу, рамкам или принятому дополнению | Отклоняется с указанием причины; никогда не попадает в промпт |
...
| `unclassified` | Его классифицировать не удалось | Отклоняется: цель прежде всего |

Принятые и отклонённые дополнения записываются (`study.amendment_accepted`, `study.amendment_refused`) и перечисляются в `study.amendments` и в отчёте. Инструкции никогда не накапливаются незаметно: каждая получает номер, подчинена цели и видна.

### Почему это работает {#why-this-works}

Дрейф возникает из-за растущего контекста: предыдущие ответы, наслоившиеся инструкции и побочные обсуждения в итоге весят больше, чем цель. Исследование устраняет этот рост. Модель никогда не перечитывает свои прежние ответы, поэтому её не может увлечь собственный дрейф. Инструкции не накапливаются: существуют только принятые дополнения, каждое с номером и подчинённое уставу, который не может измениться. Устав открывает каждый промпт, а цель его закрывает — там, где модель уделяет больше всего внимания. Каждый элемент должен оправдать себя перед целью, поэтому элемент, ушедший в сторону, легко заметить. А судья с узким обзором — устав и элементы, ничего больше — ловит то, что всё же проскальзывает, а журнал дрейфа показывает вам, что он удалил и почему.
...
Остановившийся запуск **сохраняет всё, что успел сделать**: уже выполненные этапы, элементы текущего этапа (те, которые страж ещё не оценил, помечаются как `unchecked`, предупреждение `uncheckedItems`), а также отчёт и досье, в которых сказано, что не было выполнено.
...
Политики бюджета и тайм-аута SDK (`defaultPolicies`, `defineGlobalPolicy`) проверяются **перед каждым этапом**, так же как у когнитивного агента — перед каждым шагом (см. [Лимиты и политики](./cognitive-agents#limits-and-policies)): `maxSteps` считает уже выполненные этапы, `maxTokens` — токены вызовов модели этого запуска, `maxDuration` — время с начала запуска, а `budgetLimit` с `maxTokens` или `maxCost` — его бюджет за период. Отказавшая политика записывает `policy.violated` с `passage`, и запуск останавливается: `stopped`, `stoppedBy: 'policy'`. Поиски как вызовы инструментов тоже проходят через политики.
...
| `failed` | Запуск завершила ошибка, например ответ, который не удалось использовать даже после исправления, или проектирование, в котором меньше двух корректных архитектур (`error`) | `study.failed`, `run.failed` |
...
- **Возобновление.** `run()` начинает с первого незавершённого этапа, поэтому остановленный, неудавшийся или отменённый запуск можно возобновить, снова вызвав `run()`. Уже завершённые этапы сохраняются; `study.started` записывает, с какого места запуск был возобновлён (`resumeAt`). `restart: true` снова выполняет все этапы (идентификаторы продолжаются с того места, где остановились: за `O2` следует `O3`).
...
Сам пример принимает команду любого поискового сервера MCP: запустите его командой `OPENAI_API_KEY=… SEARCH_MCP="npx -y @modelcontextprotocol/server-brave-search" BRAVE_API_KEY=… npm run example:study` (`SEARCH_TOOLS` выбирает часть инструментов сервера, `MODEL` — модель). Он записывает досье в `examples/study-navigateur.md`. Без `SEARCH_MCP` он работает без источников: всё остаётся гипотезой, и досье сообщает об этом в самом начале.
...
Отчёт также содержит устав и его хеш, дополнения, состояние каждого этапа (`complete`, `partial`, `unchecked` или `notRun`, с его попытками и этапами, которые его вновь открывали), все коллекции этапов, три состояния, сгруппированные по частям, поиски и запуски исследования (`runIds`, включая запуски дополнений). Его типы перечислены в [справочнике API SDK](../reference/sdk-api#studies).

**Предупреждения** говорят, что читателю нужно знать, прежде чем доверять остальному:
...
| `uncheckedItems` | Элементы, которые страж не оценил, потому что запуск остановился раньше |
| `searchesSkipped` | Бюджет поисков закончился, и на каких этапах |
| `leadsNotVerified` | Направления без вердикта |
| `analoguesNotDeconstructed` | Названные прорывы, которые не были разобраны |
...
6. варианты проектирования: каждая архитектура помечена как новая возможность или улучшение, с указанием, для кого она, снятого ограничения, смены принципа, механизма, пути компоненты → сборка → возможность, её условий, выгоды, дополнительной стоимости, контрпримера, охвата цепочки и предсказаний; затем — что ново, а что нет;
7. эксперименты и карточки механизмов;
8. журнал дрейфа, источники и статистика.

Каждое утверждение показывает свой статус и результаты, на которые ссылается (`S1, S3`); статус, который исследование понизило, указывает, что заявила модель и почему; новшество показывает свои предшествующие работы или то, что его ещё нужно проверить. Слова досье существуют на одиннадцати языках этой документации; для другого языка используются английские подписи, хотя модель по-прежнему пишет свои тексты на этом языке. `message` каждого предупреждения в отчёте написано по-английски; досье пишет его на своём языке.
...
- **Оно знает только то, что возвращают его источники.** У SDK нет собственного веб-поиска; без источников каждое утверждение — гипотеза.
- **Проверяется ссылка, а не её содержание.** Код проверяет, что результат, на который ссылается утверждение `established`, получен этим исследованием, но не то, что результат говорит то же, что и утверждение. Досье перечисляет каждый источник со ссылкой на него: читайте их.
...
- **То, что оно читает, не заслуживает доверия.** Результаты поиска могут содержать инструкции, нацеленные на модель (prompt injection, внедрение промпта). Исследование может вызывать только свои источники, и только через политики, а текст модели и источников экранируется в досье; статусы и правила дрейфа обеспечиваются кодом, а не промптом.
