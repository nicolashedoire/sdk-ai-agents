...
Поиск предшествующих работ, завершившийся ошибкой из-за того, что сервис ограничил частоту запросов, — ошибка его инструмента говорит `throttled`, как это делают веб-инструменты SDK, — из-за этого не пропадает: он повторяется ещё раз, после остальных поисков предшествующих работ и после паузы, которую запросили сервисы (не менее 5 с и не более 150 с), если у запуска ещё остались бы минута и поиски. Вторая попытка записывается как отдельный поиск (`retry: true`). Поиск, завершившийся ошибкой по другой причине, не повторяется, а утверждение, у которого вторая попытка тоже завершилась ошибкой, остаётся требующим проверки, как описано выше.
