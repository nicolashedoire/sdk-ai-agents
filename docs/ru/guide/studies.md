...
Поиск предшествующих работ, завершившийся ошибкой из-за того, что сервис ограничил частоту запросов, — ошибка его инструмента говорит `throttled`, как это делают веб-инструменты SDK, — из-за этого не пропадает: он повторяется ещё раз, после остальных поисков предшествующих работ и после паузы, которую запросили сервисы (не менее 5 с), если у запуска ещё остались бы минута и поиски. Поиск, сервис которого запросил больше 150 с или больше, чем осталось у запуска, не повторяется раньше, чем тот запросил. Вторая попытка записывается как отдельный поиск (`retry: true`). Поиск, завершившийся ошибкой по другой причине, не повторяется, а утверждение, у которого вторая попытка тоже завершилась ошибкой, остаётся требующим проверки, как описано выше.
