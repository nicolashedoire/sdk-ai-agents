...
| `schema` | Аргументы в виде схемы zod; тексты `.describe()` показываются модели. Вызов, который ей не соответствует, отклоняется раньше всего остального. |
| `handler(params, context?)` | Ваш код. `context` содержит `runId`, `agentId` и `signal`, который прерывается, когда вызывающая сторона сдаётся. |
...
| `databaseTools({ database })` | Перечислять таблицы, описывать одну из них, выполнять один `SELECT` (по умолчанию 100 строк) | `list_tables`, `describe_table`, `query` | средний, только для чтения | `sqliteReadOnly(db)` (`node:sqlite` или `better-sqlite3`) или `postgresReadOnly({ pool })` (`pg`) | [База данных только для чтения](./mcp-recipes#a-read-only-database) |
| `await openApiTools({ spec })` | Вызывать веб-API, по одному инструменту на операцию; только `GET`, если другие операции не перечислены в `include` | `operationId`, иначе метод и путь (`get_pets_petId`) | `GET`: низкий, только для чтения. Остальные: высокий, требуется одобрение | Описание OpenAPI 3 (URL, файл или объект) | [Веб-API](./mcp-recipes#a-web-api-from-its-openapi-description) |
...
4. **Одобрение**, если его требует инструмент или политика.
...
`maxTokens` и `maxCost` учитывают вызовы модели и, когда лимит исчерпан, отклоняют вызовы инструментов. См. [Затраты на API](./costs#budgets).
...
| Ответы из базы данных SQL без какого-либо риска записи | `databaseTools` с `sqliteReadOnly` или `postgresReadOnly` |
