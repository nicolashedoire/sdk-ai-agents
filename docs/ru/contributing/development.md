...
- Используется Vitest
- Ни один тест не обращается к настоящему платному сервису. Наборы тестов новых модулей не подменяют модули: порты SDK реализуются тестовыми дублёрами из `src/__tests__/support/`, а HTTP-адаптеры работают с локальными серверами
- Более старые наборы тестов (точка входа SDK, провайдеры OpenAI и Anthropic, движок рассуждения, резерв, запуск регрессионных тестов) по-прежнему подменяют библиотеки поставщиков `openai` и `@anthropic-ai/sdk` через `vi.mock`, а некоторые зависимости, например хранилище событий, — через `vi.fn()`

### Интеграционные тесты {#integration-tests}

- Интеграционные тесты для полных сценариев работы
- Провайдеры LLM заменяются имитацией библиотеки поставщика (`vi.mock`); ни один тест не обращается к настоящему API
...
1. Проверьте, что тестовые дублёры из `src/__tests__/support/` или имитации в более старых наборах тестов по-прежнему соответствуют интерфейсам, которые они заменяют
