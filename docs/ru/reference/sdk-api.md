...
| `providerConfig` | `{ openai?, anthropic? }` | Настройки каждого провайдера: `apiKey`, `defaultModel` и `baseURL` (совместимая конечная точка, например API v1 Azure OpenAI или локальный сервер моделей, либо прокси). Используются основным провайдером и резервными провайдерами того же поставщика |
| `fallbackProviders` | `Array<{ provider, config? }>` | Пробуются по порядку, когда основной провайдер даёт сбой; `config` имеет приоритет над `providerConfig` своего провайдера. Резервному провайдеру другого поставщика нужен собственный `apiKey` |
...
Всё, что использует SDK, экспортируется для пользовательских конфигураций: `JevClient`, `DecisionService`, `LLMThoughtGenerator`, `HeuristicController`, `TypedDecisionController`, `TypedHypothesisAssessor`, `PredictionTester`, `applyThought`, `assembleThought`, `assessReadiness`, `rankHypotheses`, `rebuildMentalState`, `describeMentalState`, `fingerprint`, `defineThinkerProfile`, `refineProfile`, `withRetry`, `RetryingLLMProvider`, `MonitoredEventStore`, `EmailIncidentNotifier`, `WebhookIncidentNotifier`, `ResendEmailTransport`, `computeRunCost`, `FileEventStore`, `SQLiteEventStore`, `PostgreSQLEventStore`, а также все типы.
