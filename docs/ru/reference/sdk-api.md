...
Что возвращает `getRunCost(runId)`: каждый вызов модели в запуске, на который поставщик ответил, включая шаги, завершившиеся ошибкой, — см. [Затраты на API](../guide/costs).
...
| `lines` | По одной на модель и источник: вызовы, токены вызовов, которые их сообщили, `unmeteredCalls`, если такие есть, `costUsd`, если у модели есть цена; `model` равно `unknown` для вызова, не записавшего имя модели |
