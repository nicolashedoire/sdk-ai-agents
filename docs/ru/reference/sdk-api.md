...
| `normalizeUrl(url)` | `string \| undefined` | URL, по которому узнаётся результат поиска: без параметров отслеживания, фрагмента и завершающей косой черты; `undefined` для всего, кроме http(s) |
