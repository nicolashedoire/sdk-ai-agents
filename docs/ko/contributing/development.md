...
- 모듈마다 단위 테스트
- Vitest 사용
- 실제 유료 서비스를 호출하는 테스트는 없습니다. 최근 모듈의 테스트 스위트는 모듈 모킹을 사용하지 않습니다. SDK 포트는 `src/__tests__/support/`의 테스트 대역이 구현하고, HTTP 어댑터는 로컬 서버를 상대로 실행됩니다
- 이전 테스트 스위트(SDK 진입점, OpenAI 및 Anthropic 프로바이더, 추론 엔진, 폴백, 회귀 테스트 실행기)는 여전히 벤더 라이브러리 `openai`와 `@anthropic-ai/sdk`를 `vi.mock`으로, 이벤트 저장소 같은 일부 의존성을 `vi.fn()`으로 대체합니다

### 통합 테스트 {#integration-tests}

- 전체 작업 흐름에 대한 통합 테스트
- LLM 프로바이더는 모킹된 벤더 라이브러리(`vi.mock`)로 대체되며, 실제 API를 호출하는 테스트는 없습니다
- 여러 이벤트 저장소를 사용한 테스트
...
1. `src/__tests__/support/`의 테스트 대역이나 이전 테스트 스위트의 목(mock)이 대체하는 인터페이스와 여전히 일치하는지 확인하세요
2. 의존성이 최신인지 확인하세요
