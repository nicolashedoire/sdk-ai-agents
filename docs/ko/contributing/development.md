...
- 모듈마다 단위 테스트
- Vitest 사용
- 실제 유료 서비스를 호출하는 테스트는 없으며, 모듈 모킹이나 스파이를 사용하는 테스트도 없습니다. SDK 포트는 `src/__tests__/support/`의 테스트 대역이 구현하고, HTTP 어댑터(OpenAI 및 Anthropic 프로바이더 포함)는 로컬 서버를 상대로 실행됩니다. `no-mocks.test.ts`는 `vi.mock`, `vi.fn`, `vi.spyOn`을 거부합니다

### 통합 테스트 {#integration-tests}

- 전체 작업 흐름에 대한 통합 테스트
- 모델은 스크립트로 동작하는 프로바이더이거나, 벤더 형식으로 응답하는 로컬 서버와 통신하는 실제 OpenAI 또는 Anthropic 클라이언트이며, 실제 API를 호출하는 테스트는 없습니다
- 여러 이벤트 저장소를 사용한 테스트
...
1. `src/__tests__/support/`의 테스트 대역이 대체하는 인터페이스와 여전히 일치하는지 확인하세요
2. 의존성이 최신인지 확인하세요
