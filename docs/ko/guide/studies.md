...
서비스가 요청 속도를 제한해서 실패한 선행 기술 검색(SDK의 웹 도구처럼, 그 도구의 오류가 `throttled`라고 밝히는 경우)은 그 때문에 버려지지 않습니다. 다른 선행 기술 검색들이 끝난 뒤, 서비스들이 요구한 대기 시간(최소 5초, 최대 150초)이 지나면 한 번 더 시도합니다. 단, 실행에 아직 1분과 남은 검색이 있을 때에 한합니다. 두 번째 시도는 별개의 검색으로 기록됩니다(`retry: true`). 다른 이유로 실패한 검색은 다시 시도하지 않으며, 두 번째 시도마저 실패한 주장은 앞에서 말한 대로 검증 대상으로 남습니다.
