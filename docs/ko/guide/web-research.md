...
- **PDF**의 텍스트는 선택 패키지 [`unpdf`](https://github.com/unjs/unpdf)(Node.js 22 이상)를 설치한 경우에만 읽습니다. `npm install unpdf`로 설치하세요. 이 패키지가 없으면 `web_fetch`는 PDF를 거부하고 설치 방법을 알려 줍니다. 제목과 날짜는 문서에서 가져옵니다. PDF는 프로세스 안에서 한 번에 하나씩, 각각 워커 스레드에서 읽습니다. pdf.js가 PDF를 읽기 전에, 워커는 자체 PDF 파서로 그 PDF를 읽습니다. 모든 객체, 각 스트림의 필터(`/Filter` 또는 `/F`, 참조는 해석해서), 그리고 그 데이터의 선언된 길이를 읽습니다. 그다음 모든 스트림을 각자의 필터(Flate, Brotli, LZW, RunLength와 이들이 연결된 경우, 그리고 이들을 감싸는 ASCII85와 ASCIIHex)로 풀어 보고, 합계가 256 MB를 넘으면 PDF를 거부합니다. 이 측정은 프로세스의 나머지 부분에 아무것도 기대지 않으므로, 기가바이트 단위로 부풀어 오르는 작은 PDF는, 사전 검사가 읽을 수 있는 것이라면, 메인 스레드가 바쁠 때도 거부됩니다. 읽을 수 없는 것도 거부합니다. 해석할 수 없는 스트림 딕셔너리, 필터나 길이, 손상된 압축 데이터, 알 수 없는 필터, 그리고 이미지의 마지막 필터 자리가 아닌 곳에 있는 이미지 필터(DCT, JPX, JBIG2, CCITT)입니다. 텍스트 추출은 이미지 데이터를 절대 디코딩하지 않으므로, 이미지 데이터 자체는 측정하지 않습니다. 암호화된 PDF는 스트림이 암호문이라 측정할 수 없습니다. 이런 PDF는 읽기는 하되, 두 번째 방어선으로만 제한됩니다. 두 번째 방어선으로, 읽는 동안 프로세스가 1 GB 넘게 커지거나, 20초가 지나거나, 호출이 끝나면 워커가 중지됩니다. 차례를 기다리는 시간도 호출의 마감 시간에 포함됩니다.
