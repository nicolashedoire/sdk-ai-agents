...
| `StudySearch` | `passage`, `purpose`(`research` 또는 `priorArt`), `tool`, `query`, `servesObjective`, `claims?`, `resultIds`, `error?`, `throttled?`(요청 속도 제한으로 실패함), `retry?`(요청 속도 제한에 걸린 선행 기술 검색의 두 번째 시도), `skipped?`(`maxSearches`), `runId` |
...
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | `SearchProvider` | 키가 필요 없는 `web_search`의 기본 프로바이더. DuckDuckGo의 HTML 페이지를 쓰며, 검색 사이에 4초를 둡니다(이전 검색이 끝난 때부터 셉니다) |
...
`web_search`는 프로바이더들에게 순서대로 묻습니다. 예외를 던진 프로바이더는 다음 프로바이더에게 차례를 넘깁니다. `SearchThrottledError`(`retryAfterMs?`: 요구한 대기 시간)를 던지거나 HTTP 429로 답한 프로바이더는, 그 대기 시간이나 `throttleWaitMs`(10초)만큼 기다린 뒤 한 번 더 시도할 기회를 얻습니다. 호출의 마감 시간에 그만한 여유가 있을 때에 한하며, 30초보다 긴 대기를 요구했을 때는 이 기회가 없습니다. 그래도 요청 속도 제한에 걸려 있으면(또는 세 번 연속 실패하면) `circuitBreaker.cooldownMs`(2분) 동안, 또는 요구한 시간이 그보다 길면 그 시간 동안 건너뜁니다. 한 호스트로 가는 요청은 프로세스 전체에서 한 번에 하나씩, 각각 이전 요청이 끝나고 `minIntervalMs`가 지난 뒤에 보냅니다(DuckDuckGo는 기본값 4초). 웹 도구는 일부러 거부한 것에 대해 `WebRequestRefusedError`(`reason`: `private-address`, `scheme`, `downgrade`, `redirects`, `robots`, `content-type`, `too-large`, `unreadable`, `pacing`)를, 2xx가 아닌 응답에 대해 `WebHttpError`(`status`)를, 요청이나 호출의 마감 시간 또는 추출의 시간 예산을 넘으면 `WebTimeoutError`를, 설정이 빠졌을 때(`unpdf`, GitHub 토큰) `WebConfigurationError`를, 어느 프로바이더도 답하지 않았을 때 `SearchUnavailableError`(`failures`, 모두 요청 속도 제한에 걸렸을 때 `throttled`, 건너뛴 프로바이더를 언제 다시 시도할지 `retryAfterMs?`)를 던집니다. `retry`는 거부, 빠진 설정, 어느 프로바이더도 답하지 않은 검색, 요청 속도 제한(HTTP 429, `SearchThrottledError`)을 절대 재시도하지 않습니다.
