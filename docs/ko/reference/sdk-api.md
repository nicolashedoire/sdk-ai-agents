...
| `retry` | `Partial<RetryPolicy> \| false` | LLM 재시도 정책, 프로바이더별로, 폴백 전에. Jev 클라이언트 재시도의 기본값이기도 함(`jev.maxRetries`와 `jev.retryBaseDelayMs`로 덮어씀). 주입한 `llmProvider`에는 명시적으로 설정한 경우에만 적용되며, `llmProvider`로 넘긴 `FallbackProvider`와 그 안의 프로바이더에는 절대 적용되지 않음 |
