...
| `cognition.thought` | `step`, `operation`, `patch`, `issues`, `failed`, `ignoredFields?`, `model?`, `requestedModel?`, `usage?`(`promptTokens`, `completionTokens`, `calls`, `unmeteredCalls?` — 토큰 수를 보고하지 않은 호출, `totalOnlyTokens?` — 그중 일부가 단독으로 보고한 합계) |
