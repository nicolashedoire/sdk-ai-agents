...
| `study.search` | `passage`, `purpose`(`research`, 또는 신규 주장의 선행 기술이면 `priorArt`), `tool`, `query`, `servesObjective`, `claims?`(찾고 있는 신규 주장), `resultIds`, `results`(`id`, `title`, `locator`, `date?`), `error?`(검색 실패), `throttled?`(요청 속도 제한으로 실패), `retry?`(요청 속도 제한에 걸린 선행 기술 검색의 두 번째 시도), `skipped?`(`maxSearches`: 실행되지 않음) |
