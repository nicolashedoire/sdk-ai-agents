...
| `amend(text, { signal?, timeoutMs? })` | `Promise<StudyAmendment>`. Allein gegenüber der Charta eingeordnet, nie gegenüber früheren Nachträgen (zwei, die einander widersprechen, können beide angenommen werden), einer nach dem anderen in der Reihenfolge der Aufrufe, in einem eigenen Lauf (`mode: 'study-amendment'`), in dem zuerst die Budget-Richtlinien geprüft werden; nur `refines` wird angenommen und erscheint in jedem späteren Prompt. `timeoutMs` (Standard 60 000, ab dem Zeitpunkt gezählt, an dem er an der Reihe ist) und `signal` begrenzen die Einordnung: Darüber hinaus, oder wenn eine Richtlinie sie ablehnt, wird der Nachtrag als `unclassified` abgelehnt. Wirft einen `ValidationError` bei einem leeren Text, einem Text von mehr als `MAX_AMENDMENT_LENGTH` (500 Zeichen) oder wenn er an die Reihe kommt, nachdem bereits `MAX_AMENDMENTS` (10) Nachträge angenommen wurden |
...
| `priorArtNotSearchedYet`, `priorArtNoSource`, `priorArtSearchBudget`, `priorArtNotSearched`, `priorArtSearchFailed`, `priorArtNoResult`, `priorArtNotAssessed`, `priorArtUnsupported` | Warum der Stand der Technik einer Neuheit oder der Zusammensetzung einer Fähigkeit noch zu prüfen ist (ihr englischer Text beginnt mit „To verify against prior art:“) |
| `priorArtExists` | Eine Neuheit, zur `hypothesis` herabgestuft: Die nächstliegende Arbeit tut das bereits |
| `assemblyExists` | Eine Fähigkeit, die keine Neuheit ist und deren Zusammensetzung es bereits gibt (in ihrem `priorArtReason`) |
| `componentDocumented`, `componentUndocumented` | Eine als neu dargestellte Komponente |
| `noServesObjective`, `invalidItem`, `notAnObject`, `notAUserLead` | Ein vom Schema abgelehntes Element |
| `leadAlreadyJudged` | Ein erneut abgegebenes Urteil zu einer bereits beurteilten Spur: verworfen, keine Drift (`duplicates` von `study.passage_completed`) |
...
| `StudyCombination` | `a`, `b`, `enables` (was A B ermöglicht), `exchange`, `cost`, `changes` (`representation`, `distribution`, `responsibilities`, `trust`, `verification`, `other`) |
...
| `StudySearch` | `passage`, `purpose` (`research` oder `priorArt`), `tool`, `query`, `servesObjective`, `claims?`, `resultIds`, `error?`, `throttled?` (sie ist an einer Drosselung gescheitert), `retry?` (der zweite Versuch einer gedrosselten Recherche zum Stand der Technik), `skipped?` (`maxSearches`), `runId` |
| `StudyPassageState` | `passage`, `state` (`complete`; `partial`: beurteilt, aber nicht beendet, weil sie auf ihre Schleife wartet oder ihre Recherche zum Stand der Technik unterbrochen wurde; `unchecked`: Elemente, die der Wächter nicht beurteilt hat; `notRun`), `attempts` (2 nach einer Wiederholung), `keptAttempt?` und `discarded?` (`attempt`, `items`: eine Wiederholung, die zugunsten eines besseren ersten Versuchs verworfen wurde), `reopenedBy`, `runId?` |
| `StudyNotice` | `code` (`noSources`, `stopped`, `failed`, `cancelled`, `passagesNotRun`, `uncheckedItems`, `searchesSkipped`, `leadsNotVerified`, `analoguesNotDeconstructed`, `noDesign`, `noCapability`, `minimumsNotMet`, `untracedAssembly`, `passagesOutdated`, `capabilitiesToVerify`, `capabilitiesExist`, `noveltiesToVerify`), `params?` (`limit`, `error`, `count`), `details?` (die betroffenen Phasen, Paare `passage.collection`, Spuren, Durchbrüche oder Architekturen), `message` (auf Englisch; das Dossier gibt den Code in seiner Sprache wieder) |
...
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | `SearchProvider` | Der Standardanbieter von `web_search`, ohne Schlüssel: die HTML-Seite von DuckDuckGo, 4 s zwischen zwei Suchen, gezählt ab dem Ende der vorherigen |
...
`web_search` fragt seine Anbieter der Reihe nach; ein Anbieter, der einen Fehler wirft, übergibt an den nächsten. Einer, der `SearchThrottledError` wirft (`retryAfterMs?`, die verlangte Wartezeit) oder mit HTTP 429 antwortet, erhält nach dieser Wartezeit oder `throttleWaitMs` (10 s) einen weiteren Versuch, wenn die Frist des Aufrufs dafür Raum lässt, und keinen, wenn er mehr als 30 s verlangt hat; wird er noch immer gedrosselt (oder schlägt er dreimal in Folge fehl), wird er für `circuitBreaker.cooldownMs` (2 Minuten) übersprungen, oder so lange, wie er verlangt hat, wenn das länger ist. Anfragen an einen Host laufen einzeln nacheinander, jede `minIntervalMs` nach dem Ende der vorherigen, für den ganzen Prozess (DuckDuckGo: standardmäßig 4 s). Die Web-Tools werfen `WebRequestRefusedError` für das, was sie absichtlich ablehnen (`reason`: `private-address`, `scheme`, `downgrade`, `redirects`, `robots`, `content-type`, `too-large`, `unreadable`, `pacing`), `WebHttpError` für eine Antwort außerhalb von 2xx (`status`), `WebTimeoutError` (eine Anfrage, die Frist des Aufrufs oder das Zeitbudget einer Extraktion), `WebConfigurationError` für eine fehlende Einrichtung (`unpdf`, ein GitHub-Token) und `SearchUnavailableError`, wenn kein Anbieter geantwortet hat (`failures`; `throttled`, wenn alle gedrosselt wurden, `retryAfterMs?`, wenn ein übersprungener wieder versucht wird). `retry` wiederholt nie eine Ablehnung, eine fehlende Einrichtung, eine Suche, auf die kein Anbieter geantwortet hat, oder eine Drosselung (HTTP 429, `SearchThrottledError`).
