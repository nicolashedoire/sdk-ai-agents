...
| `retry` | `Partial<RetryPolicy> \| false` | Wiederholungsrichtlinie für das LLM, pro Anbieter, vor dem Fallback. Sie ist auch der Standard für die Wiederholungen des Jev-Clients (`jev.maxRetries` und `jev.retryBaseDelayMs` überschreiben sie). Gilt für einen eingesetzten `llmProvider` nur, wenn sie ausdrücklich gesetzt ist, und nie für einen als `llmProvider` übergebenen `FallbackProvider` oder dessen Anbieter |
