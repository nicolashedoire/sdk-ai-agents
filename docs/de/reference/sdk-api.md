...
| `lines` | Eine pro Modell, angefragtem Modell und Quelle: Aufrufe, Tokens der Aufrufe, die sie gemeldet haben, `unmeteredCalls`, falls vorhanden, `totalOnlyTokens` für die Summen, die einige allein gemeldet haben, `costUsd`, wenn das Modell einen Preis hat und Aufrufe der Zeile ihre Tokens gemeldet haben; `model` ist `(unknown)` für einen Aufruf, der keinen Modellnamen aufgezeichnet hat |
