...
Der Stand der Technik einer Fähigkeit, die keine Neuheit ist, wird ebenfalls festgehalten, und ihr Status ändert sich nicht: Die Studie stuft Status nur herab, nie herauf. Gibt es ihre Zusammensetzung bereits, behält sie `kind: 'capability'`, ihr `priorArtReason` sagt es (`assemblyExists`, mit der nächstliegenden Arbeit), und sie wird nach den anderen Fähigkeiten eingereiht, vor den Verbesserungen (Hinweis `capabilitiesExist`).

Eine Behauptung, deren Stand der Technik nicht recherchiert oder bewertet werden konnte, bleibt zu prüfen (`toVerify`), mit Angabe des Grundes: in ihrem `statusReason` bei einer Neuheit, in ihrem `priorArtReason` bei einer Fähigkeit mit einem anderen Status. Die Gründe: Ihre Suche ist noch nicht gelaufen (`priorArtNotSearchedYet`), keine Quelle (`priorArtNoSource`), keine Suche für sie verlangt (`priorArtNotSearched`), ihre Suchen sind fehlgeschlagen (`priorArtSearchFailed`) oder haben nichts gefunden (`priorArtNoResult`), das Suchbudget war aufgebraucht (`priorArtSearchBudget`), ihre Ergebnisse wurden nicht bewertet (`priorArtNotAssessed`) oder die Prüfung hat keines ihrer eigenen Ergebnisse angeführt (`priorArtUnsupported`). Eine Neuheit, die erst nach dem Entwurf beansprucht wurde, bleibt ebenfalls zu prüfen. Der Bericht zählt sie (Hinweise `noveltiesToVerify` und `capabilitiesToVerify`).

Eine Recherche zum Stand der Technik, die fehlgeschlagen ist, weil der Dienst sie gedrosselt hat – der Fehler ihres Tools sagt `throttled`, wie es die Web-Tools des SDK tun –, geht dadurch nicht verloren: Sie wird noch einmal versucht, nach den anderen Recherchen zum Stand der Technik und nach der Wartezeit, die die Dienste verlangt haben (mindestens 5 s), wenn dem Lauf dann noch eine Minute und Suchen blieben. Eine Suche, deren Dienst mehr als 150 s verlangt hat oder länger, als dem Lauf noch bleibt, wird nicht früher erneut versucht, als er verlangt hat. Der zweite Versuch wird als eigene Suche festgehalten (`retry: true`). Eine Suche, die aus einem anderen Grund fehlgeschlagen ist, wird nicht wiederholt, und eine Behauptung, deren zweiter Versuch ebenfalls fehlschlägt, bleibt wie oben zu prüfen.

## Recherche über Ihre Quellen {#research-through-your-sources}

Eine Studie sucht mit **den Tools, die Sie ihr als `sources` geben**: den Namen von SDK-Tools. Die [Web-Tools](./web-research) des SDK funktionieren ohne Einrichtung – `web_search` (DuckDuckGo, bis Sie einen anderen Anbieter konfigurieren), `arxiv_search`, `wikipedia_search` und `github_search` –, und ihre Ergebnisse kommen mit einer URL und, falls bekannt, einem Datum:
...
Jedes andere Such-Tool funktioniert ebenfalls, zum Beispiel die Such-Tools eines MCP-Servers, die mit [`connectMcpServer`](./mcp#use-the-tools-of-an-mcp-server-in-your-agents) importiert und mit `sdk.defineTool` definiert wurden:
...
- **Nummerierte Ergebnisse.** Die Studie liest die Ergebnisse unabhängig von ihrer Form (eine Liste, ein Objekt, das eine solche enthält, etwa `results` oder `items`, JSON-Text, MCP-Textteile, Text aus Blöcken von Zeilen `Title:`, `Description:` und `URL:` – ein Ergebnis pro Block, wie MCP-Suchserver oft antworten, wobei der Fließtext unter einem Block in dessen Auszug kommt – oder reiner Text). Eine Antwort wie „No results found“ ist kein Ergebnis. Sie behält einen Titel, eine URL oder eine andere Fundstelle, ein Datum, falls angegeben, und einen Auszug, jeweils auf einer Zeile, und nummeriert sie einmal für die ganze Studie, bis zu einem Neustart: Dasselbe Ergebnis behält, wenn es erneut gefunden wird, anhand seiner Fundstelle seine Kennung. Sie behält `limits.maxResultsPerSearch` Ergebnisse jeder Suche (standardmäßig 5).
- **Als Daten vorgelegt.** Jeder Text von außerhalb der Studie – die Suchergebnisse, die Beschreibungen der Quellen, die abgelehnten Elemente, von denen eine Wiederholung erfährt – erreicht das Modell als beschrifteter JSON-Block zwischen einer Zeile `<<<UNTRUSTED-DATA-<id>` und einer Zeile `UNTRUSTED-DATA-<id>>>`. Die Kennung wird für jeden Prompt zufällig gezogen, sodass ein Text seinen Block nicht mit einer Markierung schließen kann, die er selbst geschrieben hat, und dem Modell wird gesagt, dass das, was zwischen den Markierungen steht, Daten sind, nie Anweisungen, denen es folgen soll. Die für diesen Schritt gefundenen Ergebnisse kommen mit ihrem Auszug; Ergebnisse, die die früheren Datensätze anführen, mit ihrer Kennung, ihrem Titel und ihrer Fundstelle. Nur die dort aufgeführten Kennungen können eine Behauptung stützen, die aus diesem Prompt heraus geschrieben wird.
...
Ihre Spuren sind zu prüfende Beispiele, keine Wahrheiten: `changes` muss jeder von ihnen ein Urteil geben – `relevant`, `partlyRelevant` oder `notRelevant`, mit Gründen –, und eine Antwort, die eine vergisst, wird einmal zurückgeschickt. Die Charta nummeriert die Spuren, und das Modell benennt jede mit ihrer Nummer, in welcher Sprache es auch schreibt; der Bericht gibt die Spur so wieder, wie die Charta sie schreibt. Eine Spur erhält ein einziges Urteil: Ein erneut zu ihr abgegebenes Urteil wird verworfen (`leadAlreadyJudged`) und in `study.passage_completed` als Duplikat aufgeführt; es ist keine Drift. Eine Spur, die noch kein Urteil hat, nachdem `changes` gelaufen ist, wird in `unverifiedLeads` aufgeführt (Hinweis `leadsNotVerified`). Werkzeuge, die die Studie selbst findet, sind `independentLeads`.
...
- **Verspätete Urteile wirken sich auf das Folgende aus.** Wenn der Wächter des nächsten Laufs Elemente einer bereits abgeschlossenen Phase behält, holen die Phasen, die ohne sie gelaufen sind, das nach: Ein Entwurf erhält die Recherche zu ihrem Stand der Technik, und die späteren Phasen, die sie lesen, laufen erneut, wie bei einer Schleife (`limits.maxLoops`; `study.passage_started` sagt `outdated: true`). Bleibt keine Schleife mehr übrig, bleiben diese Phasen veraltet (Hinweis `passagesOutdated`), und ein späterer Lauf schreibt sie neu.
- Wenn die abgelehnten Elemente (vom Wächter oder vom Schema) mehr als einen bestimmten Anteil dessen ausmachen, was die Phase erzeugt hat – `driftThreshold`, standardmäßig ein Drittel –, wird die Phase **einmal wiederholt**, mit der Angabe, welche Elemente abgelehnt wurden und warum. Die Wiederholung ist ein eigener Schritt, den die Budget-Richtlinien zuvor prüfen. Der bessere der beiden Versuche wird behalten: beim Entwurf derjenige, der auf eine neue Fähigkeit abzielt; dann derjenige, der nach der Beurteilung die Elemente hat, die jede Sammlung braucht; dann derjenige mit mehr behaltenen Elementen; bei Gleichstand die Wiederholung. Kann der Wächter eine Wiederholung nicht beurteilen, bleibt es beim ersten Versuch. Wird der erste Versuch behalten, sagen `study.passage_completed` und der Zustand der Phase es (`keptAttempt`), ebenso, was die verworfene Wiederholung enthielt (`discarded`); das Dossier sagt es auch, und jeder Eintrag seines Drift-Protokolls zeigt seinen Versuch.
...
Angenommene und abgelehnte Nachträge werden aufgezeichnet (`study.amendment_accepted`, `study.amendment_refused`) und in `study.amendments` sowie im Bericht aufgeführt. Anweisungen häufen sich nie stillschweigend an: Jede ist nummeriert, dem Ziel untergeordnet und sichtbar. Da jeder Nachtrag allein gegenüber der Charta beurteilt wird, können zwei Nachträge, die einander widersprechen, beide angenommen werden: Jeder präzisiert die Charta, und der Wächter beurteilt danach jedes spätere Element gegenüber der Charta und allen Nachträgen. Nachträge werden einer nach dem anderen eingeordnet, in der Reihenfolge, in der sie verlangt wurden, und das `timeoutMs` jedes Nachtrags zählt ab dem Zeitpunkt, an dem er an der Reihe ist. Sie sind auch begrenzt: `amend()` wirft einen `ValidationError` bei einem Text von mehr als 500 Zeichen (`MAX_AMENDMENT_LENGTH`) oder wenn der Nachtrag an die Reihe kommt, nachdem die Studie bereits 10 Nachträge angenommen hat (`MAX_AMENDMENTS`), sodass gleichzeitige Aufrufe das Limit nicht gemeinsam überschreiten können – darüber hinaus sollte die Charta alles sagen, in einer neuen Studie.
...
- **Fortsetzen.** Ein angehaltener, fehlgeschlagener oder abgebrochener Lauf wird fortgesetzt, indem Sie `run()` erneut aufrufen. Der Wächter beurteilt zuerst, was der letzte Lauf unbeurteilt gelassen hat, und was er behält, erreicht die Phasen, die ohne es gelaufen sind (siehe [Der Wächter](#the-guardian)). Eine Phase, deren erster Versuch vor dem Anhalten beurteilt wurde, erhält dann die Wiederholung, die ihr noch zustand, nach denselben Regeln wie in einem Lauf (`study.passage_started` sagt `redo: true` und `resumed: true`); andernfalls wird sie nur noch beendet – ein Entwurf, dessen Recherche zum Stand der Technik unterbrochen wurde, setzt bei dieser Recherche wieder ein, und sein `study.passage_completed` sagt `resumed: true`. Dann laufen die nicht abgeschlossenen Phasen. Die bereits abgeschlossenen Phasen bleiben erhalten; `study.started` hält die erste Phase fest, bei der noch Arbeit übrig ist (`resumeAt`).
...
Führen Sie das Beispiel mit `OPENAI_API_KEY=… npm run example:study` aus: Es braucht keinen weiteren Schlüssel. Es kann auch mit einem MCP-Suchserver suchen, dessen Befehl Sie angeben: `SEARCH_MCP="npx -y @modelcontextprotocol/server-brave-search" SEARCH_ENV=BRAVE_API_KEY BRAVE_API_KEY=…` fügt die Tools des Servers den Quellen hinzu. `SEARCH_ENV` nennt die Variablen, die der Server braucht: Er erhält diese und eine minimale Umgebung, nie Ihren Modellschlüssel. `SEARCH_TOOLS` wählt einige der Tools des Servers aus, `MODEL` das Modell. Es schreibt das Dossier nach `examples/study-navigateur.md`, gibt aus, warum ein Lauf nicht abgeschlossen wurde, und beendet sich dann mit dem Code 1.
...
Der Bericht enthält außerdem die Charta und ihren Hash, die Nachträge, den Zustand jeder Phase (`complete`, `partial`, `unchecked` oder `notRun`, mit ihren Versuchen, den Phasen, die sie wieder geöffnet haben, und gegebenenfalls der Wiederholung, die sie verworfen hat), jede Sammlung der Phasen, die drei Zustände nach Teil gruppiert, die Suchen und die Läufe von `run()` seit dem letzten Neustart (`runIds`). `stats.runs` und `stats.modelCalls` zählen diese Läufe, und nur die Aufrufe, die der Anbieter beantwortet hat; die Nachträge werden getrennt gezählt, und ein Neustart behält sie (`stats.amendments`: wie viele eingeordnet wurden, und ihre Modellaufrufe). Die [SDK-API-Referenz](../reference/sdk-api#studies) führt seine Typen auf.
...
| `capabilitiesToVerify` | Fähigkeiten, deren Zusammensetzung nicht mit dem Stand der Technik abgeglichen wurde (`details`: ihre Kennungen) |
| `capabilitiesExist` | Fähigkeiten, deren Zusammensetzung es bereits gibt, nach den anderen eingereiht (`details`: ihre Kennungen) |
| `passagesOutdated` | Phasen, die vor Elementen geschrieben wurden, die der Wächter verspätet beurteilt hat, und noch nicht neu geschrieben wurden |
...
8. das Drift-Protokoll (jeder Eintrag mit seinem Versuch, und die verworfenen Wiederholungen), die Quellen und die Statistiken.
...
- **Sie weiß nur, was ihre Quellen liefern.** Ohne Quellen ist jede Behauptung eine Hypothese; die [Web-Tools](./web-research) lesen nur, was öffentlich ist, ohne das JavaScript einer Seite auszuführen.
