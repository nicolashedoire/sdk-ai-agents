...
- **`url`** ist normalisiert: Tracking-Parameter (`utm_*`, `fbclid`, `gclid`…), das Fragment und ein abschließender Schrägstrich werden entfernt. Wird dieselbe Seite zweimal gefunden, durch zwei Suchen oder zwei Anbieter, wird sie nur einmal behalten.
- **`id`** ist stabil: abgeleitet aus der normalisierten URL (`web:` und 16 Hexadezimalziffern ihres SHA-256), `arxiv:1706.03762`, `wikipedia:en:7266` oder `github:owner/repo`.
...
- **HTML** wird zu Markdown (oder mit `format: 'text'` zu reinem Text), ohne zusätzliche Abhängigkeit: der Hauptinhalt (`<main>`, sonst das längste `<article>`, sonst `<body>`) mit seinen Überschriften, Absätzen, Listen, absolut gemachten Links, Tabellen, Codeblöcken und Zitaten. Skripte, Styles, Formulare, Navigation, Kopf- und Fußbereiche der Seite, Randbereiche (`<aside>`), Dialoge und jedes verborgene Element werden verworfen. Der Titel stammt aus `og:title` oder `<title>`, das Datum aus den Metadaten der Seite, ihrem JSON-LD oder einem `<time>`, die Sprache aus `<html lang>`. Zeichensätze, die die Seite angibt, werden dekodiert, komprimierte Antworten ebenfalls.
- **PDF**-Text wird mit dem optionalen Paket [`unpdf`](https://github.com/unjs/unpdf) gelesen (Node.js 22 oder neuer): `npm install unpdf`. Ohne dieses Paket sagt `web_fetch` das. Titel und Datum stammen aus dem Dokument.
...
- **`truncated: true`** sagt, dass der Inhalt abgeschnitten wurde: durch `maxChars`, weil die Seite länger als `maxResponseBytes` war oder weil ein PDF mehr als `maxPdfPages` Seiten hatte.
...
`web_search` fragt seine Anbieter **der Reihe nach**: Ein Anbieter, der fehlschlägt, sein Ratenlimit erreicht oder mit einer Captcha-Seite antwortet, übergibt an den nächsten. Die Antwort sagt, welcher Anbieter geantwortet hat (`provider`) und warum die vorherigen es nicht getan haben (`errors`).
...
| --- | --- | --- | --- |
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | keine (der Standard) | bei manchen Ergebnissen | Die HTML-Seite von DuckDuckGo, keine offizielle API. Titel, Links und Snippets; Anzeigen werden übersprungen. Eine Captcha-Seite oder zweimal eine leere Seite übergibt an den nächsten Anbieter. 1,5 s zwischen zwei Suchen. |
...
**Schutzschalter (Circuit Breaker).** Ein Anbieter, der sein Ratenlimit erreicht (HTTP 429, eine Captcha-Seite), wird sofort in Ruhe gelassen, jeder andere nach drei Fehlschlägen in Folge: Zwei Minuten lang wird er ohne Anfrage übersprungen (`errors` sagt, bis wann). Danach erhält er einen weiteren Versuch. `circuitBreaker: { cooldownMs, failureThreshold }` ändert beides.

### Ihr eigener Anbieter {#your-own-provider}

Ein Anbieter ist ein Objekt mit einem Namen und einer Funktion `search`. Senden Sie jede Anfrage über den Client `web`, den sie erhält: Er wendet die Timeouts, die Byte-Obergrenzen, die Abstände zwischen den Anfragen und die Adressprüfungen an.
...
| `userAgent` | `sdk-ai-agents (+https://github.com/nicolashedoire/sdk-ai-agents)` | Wird mit jeder Anfrage gesendet; sein erstes Wort ist der Name, mit dem die Regeln von robots.txt abgeglichen werden. |
| `timeoutMs` | `15000` | Pro Anfrage, jeder Weiterleitungsschritt für sich. |
...
| `hostIntervalMs` | `1000` | Mindestabstand zwischen zwei `web_fetch`-Anfragen an denselben Host. |
...
| `cache` | `{ ttlMs: 600000, maxEntries: 200 }` | Ergebnisse, die pro Tool und Argumenten im Arbeitsspeicher behalten werden; `false` schaltet das ab. |
| `retry` | — | Wiederholungsversuche fehlgeschlagener Aufrufe (`{ maxRetries }`): bei Ratenlimits, Serverfehlern, Zeitüberschreitungen und Netzwerkfehlern, nie bei einer Ablehnung. |
| `arxiv` | `{ baseUrl: 'https://export.arxiv.org', minIntervalMs: 3000 }` | Die arXiv-API verlangt 3 s zwischen zwei Anfragen. |
| `wikipedia` | `{ baseUrl: 'https://{language}.wikipedia.org', language: 'en' }` | `{language}` wird durch die Sprache der Suche ersetzt. |
...
1. **Nichts außerhalb des öffentlichen Internets.** Loopback (`127.0.0.1`, `::1`, `localhost`), private Netzwerke (`10.x`, `172.16.x`, `192.168.x`, `fc00::/7`), Link-Local-Adressen und der Metadatendienst der Cloud (`169.254.169.254`), Carrier-Grade-NAT, Multicast und reservierte Bereiche werden abgelehnt, ebenso IPv6-Adressen, die eine dieser Adressen enthalten (`::ffff:127.0.0.1`, NAT64, 6to4). Eine IP, die in der URL steht, wird vor dem Verbinden geprüft; ein Hostname wird durch die Namensauflösung geprüft, die die Verbindung selbst verwendet, sodass jede Adresse, in die er aufgelöst wird, beim Öffnen der Verbindung geprüft wird: Eine DNS-Antwort, die sich zwischen einer Prüfung und der Verbindung ändert, kann nicht durchrutschen. Jede Weiterleitung wird erneut geprüft.
2. **Die Ausnahme ist ausdrücklich.** `allowPrivateNetwork: ['intranet.example']` lässt nur die aufgeführten Hosts durch, `true` alle. Die `baseUrl` eines Anbieters oder einer Quelle stammt aus Ihrem Code, nicht vom Modell: Sie ist sogar auf diesem Rechner erreichbar (ein SearXNG auf `localhost`), aber nur unter ihrem eigenen Ursprung – `web_fetch` lehnt sie trotzdem ab.
...
4. **Begrenzt.** Ein Timeout pro Anfrage; höchstens `maxResponseBytes` werden gelesen, nach der Dekomprimierung, und der Rest wird nie heruntergeladen; PDFs innerhalb von `maxPdfBytes` (ein PDF, das eine größere Größe ankündigt, wird abgelehnt, bevor es heruntergeladen wird) und `maxPdfPages`; Inhalt innerhalb von `maxChars`.
5. **Höflich.** `web_fetch` liest robots.txt ([RFC 9309](https://www.rfc-editor.org/rfc/rfc9309)) und ruft nie ab, was die Datei für seinen User-Agent verbietet, Weiterleitungen eingeschlossen: Es gilt die Gruppe, die seinen User-Agent nennt, sonst `*`; die längste Regel gewinnt, bei Gleichstand gewinnt `Allow`; Muster mit `*` und `$` werden verstanden. Eine fehlende robots.txt (4xx) erlaubt alles; eine, die fehlschlägt (5xx, 429) oder nicht erreichbar ist, verbietet alles. `Crawl-delay` legt den Abstand zwischen den Anfragen an ihre Website fest. Die Anfragen an jeden Host werden zeitlich verteilt (1 s für Seiten, 1,5 s für DuckDuckGo, 3 s für arXiv), Antworten werden zwischengespeichert, und der User-Agent sagt, wer fragt. Such-APIs werden nicht gecrawlt: Für sie gilt robots.txt nicht.
6. **Inhalt ist Daten.** Jede Antwort sagt `untrusted: true`, und die Tool-Beschreibungen weisen das Modell an, nie Anweisungen zu folgen, die darin stehen. Vor der Extraktion verwirft `web_fetch`, was ein Leser nicht sieht, ein Modell aber schon: Elemente, die `hidden`, `aria-hidden="true"`, `display:none` oder `visibility:hidden` sind oder eine Schriftgröße oder Deckkraft von null haben, HTML-Kommentare sowie Zeichen mit Nullbreite und bidirektionale Steuerzeichen. Eine Studie zeigt die Ergebnisse ihrem Modell zwischen Markierungen für nicht vertrauenswürdige Daten.
...
- **Die Caches liegen im Arbeitsspeicher**, getrennt für jeden Aufruf von `webTools()`, und gehen verloren, wenn der Prozess endet.
