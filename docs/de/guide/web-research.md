...
- **PDF**-Text wird mit dem optionalen Paket [`unpdf`](https://github.com/unjs/unpdf) gelesen (Node.js 22 oder neuer): `npm install unpdf`. Ohne dieses Paket sagt `web_fetch` das. Titel und Datum stammen aus dem Dokument. PDFs werden im Prozess nacheinander gelesen, jedes in einem Worker-Thread. Bevor pdf.js ein PDF liest, entpackt der Worker dessen Streams durch ihre Filter (Flate, LZW und RunLength, auch verkettet, sowie die ASCII85- und ASCIIHex-Filter, die sie umhüllen) und lehnt das PDF ab, wenn sie zusammen mehr als 256 MB ergeben: Diese Messung braucht nichts vom Rest des Prozesses, sodass ein kleines PDF, das sich auf Gigabytes aufbläht, auch dann abgelehnt wird, wenn der Haupt-Thread beschäftigt ist. Einen Stream, den er nicht messen kann (verschlüsselt oder mit einem Filter, den er nicht kennt, etwa dem eines Bildes), überlässt er pdf.js. Als zweite Linie wird der Worker gestoppt, wenn der Prozess während des Lesens um mehr als 1 GB wächst, nach 20 s oder wenn der Aufruf endet; das Warten, bis er an der Reihe ist, geht von der Frist des Aufrufs ab.
...
| `retry` | — | Wiederholungsversuche fehlgeschlagener Aufrufe (`{ maxRetries }`): bei Ratenlimits, Serverfehlern, Zeitüberschreitungen und Netzwerkfehlern; nie bei einer Ablehnung, einer fehlenden Einrichtung (`WebConfigurationError`) oder einer Suche, auf die kein Anbieter geantwortet hat (`SearchUnavailableError`). |
...
- **Die Caches und die Abstände zwischen den Anfragen liegen im Arbeitsspeicher**, getrennt für jeden Aufruf von `webTools()`, und gehen verloren, wenn der Prozess endet.
