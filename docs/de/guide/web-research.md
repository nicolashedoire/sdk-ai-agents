...
**Drosselung.** Ein Anbieter, der eine Suche drosselt (HTTP 429, die Captcha-Seite oder die leere Seite von DuckDuckGo), erhält einen weiteren Versuch, nach der Wartezeit, die er verlangt hat (`Retry-After`), oder `throttleWaitMs` (10 s), höchstens 30 s, wenn die Frist des Aufrufs dafür Raum lässt.
