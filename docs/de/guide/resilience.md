...
Ein Anbieter, den Sie mit `llmProvider` einsetzen, wird unverändert verwendet, sofern Sie `retry` nicht ausdrücklich setzen, und ein `FallbackProvider` wird nie umhüllt, damit seine Failover im Trace sichtbar bleiben. Auch seine Anbieter werden nicht umhüllt, `retry` gilt für sie also nicht: Um einen vor dem Failover erneut zu versuchen, umhüllen Sie ihn mit `RetryingLLMProvider` und geben seinem Client `maxRetries: 0`.
