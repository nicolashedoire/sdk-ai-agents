...
Bei einem Lauf, der seinen Text streamt (`onText`), kann ein Modellaufruf fehlschlagen, nachdem ein Teil der Antwort angekommen ist. Ein Fehler, den der Hersteller mitten im Stream sendet, zählt wie sein HTTP-Gegenstück: `server_error` (500) von OpenAI sowie `rate_limit_error` (429), `api_error` (500), `timeout_error` (504) und `overloaded_error` (529) von Anthropic werden wiederholt, andere Typen nicht. Eine Verbindung, die mitten im Stream abbricht, ein Stream, der endet, bevor seine Antwort vollständig ist, und ein Stream, der für die Dauer des Timeouts des Clients nichts sendet (standardmäßig 10 Minuten; `timeout` in `providerConfig`, in der `config` eines Fallbacks oder in den Optionen von `OpenAIProvider` und `AnthropicProvider`), sind Verbindungsfehler. Vor dem Wiederholungsversuch oder vor dem Fallback-Anbieter teilt `onTextRestart` dem Aufrufer mit, den Text zu verwerfen, den der fehlgeschlagene Versuch gestreamt hatte: Der nächste Versuch schreibt die Antwort neu (siehe [Die Antwort streamen](./governed-agents#_7-streaming-the-answer)).
