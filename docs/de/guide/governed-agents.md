...
- Mit `onText` rufen die eingebauten Anbieter OpenAI und Anthropic die Streaming-API ihres Herstellers auf, und Ergebnis und Ereignisse des Laufs bleiben dieselben wie ohne diese Option. Das gilt auch für seine Kosten, außer mit einem OpenAI-kompatiblen Server (über eine `baseURL`): Der Verbrauch einer gestreamten Antwort wird nur bei der eigenen API von OpenAI angefordert, es sei denn, `providerConfig.openai.includeStreamUsage` ist gesetzt, und ein Aufruf ohne Verbrauchsangabe gilt in Budgets als nicht gemessen.
