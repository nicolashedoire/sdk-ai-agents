...
| **Tool-Quelle** (Tool Source) | Eine Funktion, die aus einem System fertige Tools erstellt: `openApiTools`, `folderTools`, `databaseTools`, `webTools`, `governedAgentTool`, `cognitiveAgentTool` und `connectMcpServer` für die Tools eines beliebigen MCP-Servers. Stellen Sie sie mit einer Zeile über MCP bereit oder geben Sie sie Ihren eigenen Agenten. Siehe [Tools](./tools), [Ein MCP-Server für alles](./mcp-recipes) und [Webrecherche](./web-research). |
