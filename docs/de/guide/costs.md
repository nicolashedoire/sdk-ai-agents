...
Kosten sind nur eine Seite; Richtlinien können außerdem **Schritte, Tokens und Tool-Aufrufe** pro Agent, Tool und Zeitraum begrenzen – siehe [Kontrollierte Agenten](./governed-agents). Ein `budgetLimit` mit `maxCost` lehnt die Tool-Aufrufe eines Agenten ab, sobald seine Modellaufrufe im Zeitraum mehr als die Grenze gekostet haben, zu den obigen Preisen, und ebenso den nächsten Schritt eines kognitiven Agenten (siehe [Limits und Richtlinien](./cognitive-agents#limits-and-policies)): Ein bereits gestarteter Modellaufruf wird nie unterbrochen, und mit `toolName` wird nur dieses Tool abgelehnt. Hat ein Modell keinen Preis oder meldet ein Aufruf keine Token-Zahlen, lässt sich die Grenze nicht prüfen, und diese Tool-Aufrufe und Schritte werden abgelehnt. Ein `maxCost`, der keine endliche Zahl ≥ 0 ist (eine aus einer Konfigurationsdatei gelesene Zeichenkette wie `'0.5'`, `NaN`, ein negativer Betrag, `Infinity`, `null`), wird schon beim Anwenden der Richtlinie mit einem `ValidationError` abgelehnt.

Budgets zählen die in Läufen aufgezeichneten Modellaufrufe, wie `getRunCost`: die Reasoning-Schritte eines kontrollierten Agenten; die Gedanken eines kognitiven Agenten (einschließlich Reparaturen und fehlgeschlagener Versuche), seine Tool-Auswahl und seine typisierten Entscheidungen; sowie die mit `sdk.decisions` getroffenen typisierten Entscheidungen. Eine Grenze mit `agentId` zählt die Modellaufrufe dieses Agenten – und die Aufrufe von `sdk.decisions`, die ihn mit `agentId` nennen; eine ohne `agentId` zählt sie alle, auch typisierte Entscheidungen ohne Agenten. Ein Budget lehnt nie einen Aufruf von `sdk.decisions` ab: Es lehnt Tool-Aufrufe und die Schritte kognitiver Agenten ab.
