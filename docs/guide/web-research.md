...
- **PDF** text is read with the optional package [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 or later): `npm install unpdf`. Without it, `web_fetch` says so. The title and date come from the document. The PDF is read in a worker thread, stopped if the process grows by more than 256 MB, after 20 s, or when the call ends: a small PDF that inflates to gigabytes cannot block or exhaust the process.
...
4. **Bounded.** A deadline for the whole call (`callTimeoutMs`) and a timeout per request; at most `maxResponseBytes` read, after decompression, and the rest never downloaded; PDFs within `maxPdfBytes` (a PDF that announces a larger size is refused before it is downloaded) and `maxPdfPages`, read in a worker stopped past 256 MB or 20 s; the extraction of a page within 100,000 elements and 5 s of work; content within `maxChars`. No work after the download can block the process: the robots.txt matcher runs in linear time, and the HTML path has no quadratic step.
