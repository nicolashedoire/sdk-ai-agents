...
- **PDF** text is read with the optional package [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 or later): `npm install unpdf`. Without it, `web_fetch` says so. The title and date come from the document. PDFs are read one at a time in the process, each in a worker thread. Before pdf.js reads a PDF, the worker inflates its streams through their filters (Flate, LZW and RunLength, chained ones included, and the ASCII85 and ASCIIHex that wrap them) and refuses the PDF past 256 MB in all: this measure needs nothing from the rest of the process, so a small PDF that inflates to gigabytes is refused even while the main thread is busy. A stream it cannot measure (encrypted, or with a filter it does not know, such as an image's) is left to pdf.js. As a second line, the worker is stopped if the process grows by more than 1 GB while it reads, after 20 s, or when the call ends; waiting for its turn counts against the call's deadline.
...
4. **Bounded.** A deadline for the whole call (`callTimeoutMs`) and a timeout per request; at most `maxResponseBytes` read, after decompression, and the rest never downloaded; PDFs within `maxPdfBytes` (a PDF that announces a larger size is refused before it is downloaded) and `maxPdfPages`, read one at a time in a worker that refuses a PDF whose streams inflate past 256 MB before pdf.js reads it, and stops past 1 GB of growth or 20 s; the extraction of a page within 100,000 elements and 5 s of work; content within `maxChars`. No work after the download can block the process: the robots.txt matcher runs in linear time, and the HTML path has no quadratic step.
