...
The prior art of an architecture that is not a novelty is recorded too, and its status does not change: the study lowers statuses, it never raises them.

A novelty whose prior art could not be searched or assessed stays to verify, and its `statusReason` says why: no source (`noveltyNoSource`), no search asked for it (`noveltyNotSearched`), its searches failed (`noveltySearchFailed`) or found nothing (`noveltyNoResult`), the search budget ran out (`noveltySearchBudget`), its results were not assessed (`noveltyNotAssessed`), or the check cited none of its own results (`noveltyUnsupported`). A novelty claimed after the design also stays to verify. The report counts them (notice `noveltiesToVerify`).
...
- **Numbered results.** The study reads the results whatever their shape (a list, an object holding one such as `results` or `items`, JSON text, MCP text parts, text written as blocks of `Title:`, `Description:` and `URL:` lines — one result per block, as MCP search servers often answer — or plain text). It keeps a title, a URL or other locator, a date when given and an excerpt, each on one line, and numbers them once for the whole study, until a restart: the same result found again, by its locator, keeps its id. It keeps `limits.maxResultsPerSearch` results of each search (5 by default).
- **Shown as data.** A prompt lists the results it may cite as a JSON array between the marks `<<<UNTRUSTED-SEARCH-RESULTS` and `UNTRUSTED-SEARCH-RESULTS>>>`, and the model is told that what lies between them is data from outside sources, never instructions to follow. The results found for this step come with their excerpt; results the earlier records cite come with their id, title and locator. Only the ids listed there can support a claim written from that prompt.
...
Your leads are examples to verify, not truths: `changes` must give each one a verdict — `relevant`, `partlyRelevant` or `notRelevant`, with reasons — and a reply that forgets one is sent back once. The charter numbers the leads, and the model names each one by its number, whatever language it writes in; the report writes the lead back as the charter does. A lead gets one verdict: another verdict on it is refused (`leadAlreadyJudged`). A lead still without a verdict once `changes` has run is listed in `unverifiedLeads` (notice `leadsNotVerified`). Tools the study finds on its own are `independentLeads`.
...
Accepted and refused amendments are recorded (`study.amendment_accepted`, `study.amendment_refused`) and listed in `study.amendments` and in the report. Instructions never pile up silently: each one is numbered, subordinate to the objective, and visible. They are bounded too: `amend()` throws a `ValidationError` for a text longer than 500 characters (`MAX_AMENDMENT_LENGTH`), or once the study has accepted 10 amendments (`MAX_AMENDMENTS`) — past that, the charter should say it all, in a new study.
...
- **Resume.** A stopped, failed or cancelled run is resumed by calling `run()` again. The guardian first judges what the last run left unjudged; a passage judged but not finished then only finishes — a design whose prior-art search was cut short resumes at that search, and its `study.passage_completed` says `resumed: true` — and the passages not complete run. The passages already complete are kept; `study.started` records the first passage with work left (`resumeAt`).
