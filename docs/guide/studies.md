...
A prior-art search that failed because the service throttled it — its tool's error says `throttled`, as the SDK's web tools say — is not lost to that: it is tried once more, after the other prior-art searches and after the wait the services asked for (at least 5 s), when the run would still have a minute and searches left. A search whose service asked for more than 150 s, or for longer than the run has left, is not tried again sooner than it asked. The second try is recorded as a search of its own (`retry: true`). A search that failed for another reason is not tried again, and a claim whose second try fails too stays to verify, as above.
