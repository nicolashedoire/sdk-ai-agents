...
A run goes through the seven passages of the method, in order. Each one produces items of a few kinds (its **collections**), and each item gets an id that is never reused: `O1`, `P2`, `A1`…
...
| 6 | `design` | Designs at least two architectures, at least one aiming at a new capability, each covering the whole chain, with its mechanism, conditions, benefit, added cost, a possible counterexample and its predictions. Gives the three states of each main piece and says what is novel and what is not. Then searches the prior art of the novelties | `architectures` (`A`), `threeStates` (`T`), `noveltyClaims` (`N`) |
...
The results the searches return are numbered too: `S1`, `S2`… Each passage receives the items of the earlier passages it needs, as compact JSON records.
...
The passages form a loop. When an unknown blocks a passage — the design needs to know how a piece really works, say — it can ask to **reopen** an earlier passage on that point. The earlier passage runs again on that focus and adds items to the ones it had, then the passage that asked runs again with them. `limits.maxLoops` bounds the reopenings of a run (1 by default, 0 to allow none); a passage that was itself reopened cannot reopen another.
...
The guardian (see [The guardian](#the-guardian)) also judges each architecture: a capability it finds only faster or cheaper becomes an `improvement`, with `declaredKind: 'capability'` to show what the model claimed. A design left with no capability at all is off the objective as a whole: it is logged in the drift log and redone once; if the redo still has none, the report says so (notice `noCapability`). In the report, **capabilities come first, improvements after**.
...
- a component is **never a novelty**: one presented as new is `established` if a result the study retrieved documents it, and a `hypothesis` otherwise, with the reason;
- the architecture's own status is that of its assembly and its capability. When it claims a novelty, its prior art is searched **as a combination**: the study looks for work that already joins the same components to produce the same capability, not for each piece.

The dossier shows the path of each architecture: components (with their statuses) → assembly (with its status) → capability.
...
The breakthroughs you name in `analogues` must each be deconstructed. A reply that forgets one is sent back once; if one is still missing, the report lists it in `undeconstructedAnalogues`, with the notice `analoguesNotDeconstructed`. The study may add other breakthroughs it found.
...
| --- | --- | --- |
| `established` | It cites at least one result **this study retrieved** from its sources | It becomes a `hypothesis`. `declaredStatus` keeps the status the model gave and `statusReason` says why; ids it cited that the study never retrieved are kept apart in `unretrievedSources` and support nothing |
...
A status the study cannot read counts as `hypothesis`, never as a stronger one. **Without sources, nothing can be established**: every claim is at best a hypothesis, no novelty can be checked, and the report says so in its first notice (`noSources`).

### Prior art

After the design, the study searches the prior art of every novelty still to verify, the design's and the earlier passages' alike. The model chooses the searches (for an architecture: the combination of its components and the capability), the study runs them, then a separate call names the closest existing work among the results and gives a verdict:

- `novel` or `partlyNovel`: the claim stays a novelty, no longer to verify, with its `priorArt` (`closest`, `sources`, `verdict`);
- `exists`: the idea is already done; the claim becomes a `hypothesis`, and `statusReason` names the closest work.

A novelty whose prior art could not be searched or assessed — no source, the search budget spent, no search asked for it, or a novelty claimed after the design — stays to verify, and says why. The report counts them (notice `noveltiesToVerify`).

## Research through your sources

The SDK has no built-in Web search. A study searches with **the tools you give it** as `sources`: the names of SDK tools, typically the search tools of an MCP server imported with [`connectMcpServer`](./mcp#use-the-tools-of-an-mcp-server-in-your-agents) and defined with `sdk.defineTool`:
...
- **Numbered results.** The study reads the results whatever their shape (a list, an object holding one such as `results` or `items`, JSON text, MCP text parts, plain text), keeps a title, a URL or other locator, a date when given and an excerpt, and numbers them once for the whole study: the same result found again, by its locator, keeps its id. It keeps `limits.maxResultsPerSearch` results of each search (5 by default).
...
Your leads are examples to verify, not truths: `changes` must give each one a verdict — `relevant`, `partlyRelevant` or `notRelevant`, with reasons — and a reply that forgets one is sent back once. A lead still without a verdict is listed in `unverifiedLeads` (notice `leadsNotVerified`). Tools the study finds on its own are `independentLeads`.
...
- the compact records it needs from the earlier passages (JSON, not transcripts);
- the search results it may cite.
...
The charter, objective included, is the first thing the model reads, and the objective is the last.
...
- When the rejected items (by the guardian or by the schema) are more than a share of what the passage produced — `driftThreshold`, a third by default — the passage is **redone once**, told which items were rejected and why. The redo's items replace the first attempt's.
- The report keeps every rejection (`driftLog`), and counts them and the redos in `stats`.

### Amendments

You can add an instruction after the study was created. It never slips in unnoticed: the guardian classifies it against the charter, in a run of its own.
...
| `conflicts` | It contradicts the charter, the scope or an accepted amendment | Refused, with the reason; it never reaches a prompt |
...
| `unclassified` | Its classification failed | Refused: the objective comes first |

Accepted and refused amendments are recorded (`study.amendment_accepted`, `study.amendment_refused`) and listed in `study.amendments` and in the report. Instructions never pile up silently: each one is numbered, subordinate to the objective, and visible.

### Why this works

Drift comes from context that grows: earlier answers, stacked instructions and side discussions end up weighing more than the objective. A study removes that growth. The model never rereads its own earlier replies, so it cannot be carried along by its own drift. Instructions do not accumulate: only accepted amendments exist, each numbered and subordinate to a charter that cannot change. The charter opens every prompt and the objective closes it, where a model pays the most attention. Every item must justify itself against the objective, which makes a wandering item easy to spot. And a judge with a narrow view — the charter and the items, nothing else — catches what still slips, while the drift log shows you what it removed and why.
...
A run that stops **keeps everything it did**: the passages already done, the items of the passage in progress (those the guardian had not judged yet are marked `unchecked`, notice `uncheckedItems`), and a report and dossier that say what was not run.
...
The SDK's budget and timeout policies (`defaultPolicies`, `defineGlobalPolicy`) are checked **before each passage**, as a cognitive agent's are before each step (see [Limits and policies](./cognitive-agents#limits-and-policies)): `maxSteps` counts the passages already performed, `maxTokens` the tokens of the run's model calls, `maxDuration` the time since the run started, and a `budgetLimit` with `maxTokens` or `maxCost` its budget per period. A policy that refuses records `policy.violated` with the `passage`, and the run stops: `stopped`, `stoppedBy: 'policy'`. The searches, as tool calls, go through the policies too.
...
| `failed` | An error ended it, such as a reply that could not be used even after its repair, or a design with fewer than two valid architectures (`error`) | `study.failed`, `run.failed` |
...
- **Resume.** `run()` starts at the first passage that is not complete, so a stopped, failed or cancelled run can be resumed by calling `run()` again. The passages already complete are kept; `study.started` records where the run resumed (`resumeAt`). `restart: true` runs every passage again (the ids go on from where they were: `O3` follows `O2`).
...
The example itself takes the command of any MCP search server: run it with `OPENAI_API_KEY=… SEARCH_MCP="npx -y @modelcontextprotocol/server-brave-search" BRAVE_API_KEY=… npm run example:study` (`SEARCH_TOOLS` picks some of the server's tools, `MODEL` the model). It writes the dossier to `examples/study-navigateur.md`. Without `SEARCH_MCP`, it runs without sources: everything stays a hypothesis, and the dossier says so first.
...
The report also holds the charter and its hash, the amendments, the state of each passage (`complete`, `partial`, `unchecked` or `notRun`, with its attempts and the passages that reopened it), every collection of the passages, the three states grouped by piece, the searches, and the runs of the study (`runIds`, amendment runs included). The [SDK API reference](../reference/sdk-api#studies) lists its types.

The **notices** say what the reader must know before trusting the rest:
...
| `uncheckedItems` | Items the guardian did not judge, the run having stopped first |
| `searchesSkipped` | The search budget ran out, and in which passages |
| `leadsNotVerified` | Leads without a verdict |
| `analoguesNotDeconstructed` | Named breakthroughs not deconstructed |
...
6. the design leads: each architecture labelled new capability or improvement, with for whom, the constraint lifted, the change of principle, the mechanism, the components → assembly → capability path, its conditions, benefit, added cost, counterexample, chain coverage and predictions; then what is novel and what is not;
7. the experiments, and the mechanism cards;
8. the drift log, the sources, and the statistics.

Every claim shows its status and the results it cites (`S1, S3`); a status the study lowered says what the model declared and why; a novelty shows its prior art, or that it is still to verify. The dossier's words exist in the eleven languages of this documentation; another language gets English labels, while the model still writes its texts in that language. The `message` of each notice in the report is in English; the dossier writes it in its own language.
...
- **It knows only what its sources return.** The SDK has no Web search of its own; without sources, every claim is a hypothesis.
- **A citation is checked, not its content.** The code checks that a result cited by an `established` claim was retrieved by this study, not that the result says what the claim says. The dossier lists every source with its link: read them.
...
- **What it reads is untrusted.** Search results can hold instructions aimed at the model (prompt injection). A study can only call its sources, through the policies, and the text of the model and of the sources is escaped in the dossier; the statuses and the drift rules are enforced in code, not by the prompt.
