...
- **It fails closed.** Only a verdict with a true or false `onObjective` counts. An item left without one stays `unchecked`: it stays in the report, flagged (notice `uncheckedItems`), but never reaches a later prompt, and the next run has the guardian judge it first. A guardian that judges none of a passage's items fails the run. When a repair cannot be used, the first reply is read leniently instead: its valid verdicts count, and the items without one stay unchecked (`usedAttempt: 1` on `study.model_called`).
