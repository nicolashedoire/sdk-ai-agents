...
The SDK records the token usage of every model call **in the run that made it** — tool selection, cognitive thoughts, typed decisions and the calls of a study — and prices it per model. A call counts as soon as the vendor has answered it, even when the SDK then fails the step on its answer (see [Failed calls](#failed-calls)).
...
A call the vendor answered is billed, whatever the SDK then does with the answer. It is recorded and counted — in the run's events, in `getRunCost`, in budgets per period and in the run's `maxTokens` — even when the step fails on it:
...
- a study's call whose reply cannot be used even after its repair, or that a stop or a timeout cut short after billed attempts: `study.model_called` with `failed` and the `usage` of the attempts answered;
...
| `study.model_called` | A study's calls: passages, search requests, guardian checks, prior-art checks and amendments, repairs included (one event for a call and its repair; an answer a provider discarded is a `provider.answer_discarded`) | `model`, `requestedModel`, `usage.calls`, `usage.unmeteredCalls`, `usage.unmeteredTokens` |
