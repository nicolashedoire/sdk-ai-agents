...
| --- | --- | --- |
| `facts` | `F1…` | Statements with a source (`input`, `tool`, `inference`) and a confidence |
...
| `hypotheses` | `H1…` | Candidate answers with simulations, critiques, a support in [0, 1] and a status |
| `contradictions` | `C1…` | Conflicts between items, until resolved |
| `failures` | `X1…` | What already failed, so it is not retried blindly |
| `confidence`, `decision`, `trail` | | Overall confidence, the final decision, one line per step |
...
| `hypothesize` | Proposes new candidate answers | Fewer active hypotheses than `maxHypotheses` |
| `simulate` | Projects consequences step by step, including second-order effects | A hypothesis has no simulation |
...
| `compare` | Weighs hypotheses against evidence and the thinker's priorities | A critiqued hypothesis changed since the last comparison |
| `decide` | Commits to an answer, rationale, confidence and next actions | At least one hypothesis is still in play |
...
- the **last step is always a decision attempt** — if the model still cannot produce a valid decision, the run fails with the reason instead of looping.
...
| --- | --- | --- |
| `heuristic` | A fixed order of attention: represent → hypothesize → simulate → critique → seek information → compare → decide | Default without Jev, deterministic, free |
...
`seek_information` uses the **native reasoning and action engines**: the LLM picks one tool for the open unknown, the action engine checks that the tool was **given to this agent**, validates the call against policies, approvals and budgets, and the observation is integrated as facts with `source: "tool"`. A denied, blocked or failing tool becomes a recorded failure, and the reasoning continues.
...
sdk.createCognitiveAgent({
  ...
  limits: {
    ...
    maxToolCalls: 5,
    decisionThreshold: 0.75,   // confidence at which exploration may stop
