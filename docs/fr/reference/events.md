...
| `study.passage_started` | `passage`, `number` (1 à 7), `amendments` (numéros des amendements acceptés en vigueur), `reopenedBy?`, `focus?`, `reason?` quand un passage ultérieur l'a rouvert, `redo?` et `resumed?` pour le passage refait que lui devait une exécution reprise, `outdated?` quand il s'exécute de nouveau pour des éléments que le gardien a jugés tardivement |
| `study.passage_completed` | `passage`, `attempts` (2 quand le gardien l'a fait refaire), `keptAttempt?` (1 quand la première tentative était meilleure que le passage refait et a été conservée), `discarded?` (`attempt`, `items` : le passage refait alors écarté), `items` (chacun avec sa `collection`, son `id`, son `statement`, son `status`, ses `sources`, son `servesObjective`, les autres champs d'une affirmation et ses propres champs), `reopenedBy?`, `reopen?` (`passage`, `focus`, `reason` : le passage précédent qu'il demande de rouvrir ; il n'est pas terminé tant qu'il ne s'est pas exécuté de nouveau), `resumed?` (une exécution ne l'a repris que pour le terminer), `duplicates?` (les verdicts redonnés sur des pistes déjà jugées : ignorés, pas une dérive) |
| `study.search` | `passage`, `purpose` (`research`, ou `priorArt` pour l'existant des nouveautés), `tool`, `query`, `servesObjective`, `claims?` (les nouveautés qu'elle vise), `resultIds`, `results` (`id`, `title`, `locator`, `date?`), `error?` (la recherche a échoué), `throttled?` (elle a échoué sur une limitation de débit), `retry?` (le second essai d'une recherche de l'existant limitée en débit), `skipped?` (`maxSearches` : non exécutée) |
| `study.model_called` | `purpose` (`passage`, `queries`, `check`, `priorArtQueries`, `priorArtCheck`, `amendment`), `passage?`, `model?`, `requestedModel?`, `usage` (`promptTokens`, `completionTokens`, `calls`, `unmeteredCalls?`, `unmeteredTokens?` : un seul événement pour un appel et sa réparation), `failed?` (pourquoi la réponse n'a pas pu être utilisée), `usedAttempt?` (1 quand la réparation n'a pas pu être utilisée et que la première réponse l'a été, lue de façon permissive) — compté dans les coûts et dans les budgets par période |
