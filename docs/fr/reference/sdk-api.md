...
| `providerConfig` | `{ openai?, anthropic? }` | Réglages de chaque fournisseur : `apiKey`, `defaultModel` et `baseURL` (un endpoint compatible, comme l'API v1 d'Azure OpenAI ou un serveur de modèles local, ou un proxy). Utilisés par le fournisseur principal et par les replis du même fournisseur |
| `fallbackProviders` | `Array<{ provider, config? }>` | Essayés dans l'ordre quand le fournisseur principal échoue ; un `config` l'emporte sur le `providerConfig` de son fournisseur. Un repli d'un autre fournisseur a besoin de sa propre `apiKey` |
