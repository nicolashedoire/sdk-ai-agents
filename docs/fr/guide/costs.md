...
- l'appel d'une étude dont la réponse reste inutilisable même après sa réparation, ou qu'un arrêt ou un délai dépassé interrompt après des tentatives facturées : `study.model_called` avec `failed` et l'`usage` des tentatives qui ont reçu une réponse ;
...
| `cognition.thought` | Opérations cognitives, réparations et tentatives échouées comprises (une tentative antérieure à laquelle un autre modèle que le dernier a répondu, par un fournisseur de secours, est enregistrée comme un `provider.answer_discarded` quand elle a rapporté sa consommation) | `model`, `requestedModel`, `usage.calls`, `usage.unmeteredCalls`, `usage.unmeteredTokens` |
...
| `study.model_called` | Les appels d'une étude : passages, demandes de recherche, contrôles du gardien, évaluations de l'existant et amendements, réparations comprises (un seul événement pour un appel et sa réparation ; une réponse qu'un fournisseur a écartée est un `provider.answer_discarded`) | `model`, `requestedModel`, `usage.calls`, `usage.unmeteredCalls`, `usage.unmeteredTokens` |
