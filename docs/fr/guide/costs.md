...
Un appel auquel l'éditeur a répondu est facturé, quoi que le SDK fasse ensuite de la réponse. Il est enregistré et comptabilisé — dans les événements de l'exécution, dans `getRunCost` et, pour les agents gouvernés, dans les budgets par période et dans le `maxTokens` de l'exécution — même quand l'étape échoue à cause de lui :
