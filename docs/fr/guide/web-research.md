...
- **Le texte des PDF** n'est lu que si vous installez le paquet facultatif [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 ou plus récent) : `npm install unpdf`. Sans lui, `web_fetch` refuse un PDF et indique comment l'installer. Le titre et la date viennent du document. Les PDF sont lus un par un dans le processus, chacun dans un thread de travail. Avant que pdf.js ne lise un PDF, le thread de travail le lit avec son propre analyseur de PDF : chaque objet, les filtres de chaque flux (`/Filter` ou `/F`, références résolues) et la longueur déclarée de ses données. Il décompresse ensuite chaque flux à travers ses filtres (Flate, Brotli, LZW et RunLength, y compris enchaînés, ainsi que les ASCII85 et ASCIIHex qui les enveloppent) et refuse le PDF au-delà de 256 Mo au total : cette mesure ne dépend en rien du reste du processus, si bien qu'un petit PDF qui se décompresse en gigaoctets, dès lors que l'analyse préalable peut le lire, est refusé même quand le thread principal est occupé. Il refuse aussi ce qu'il ne sait pas lire : un dictionnaire de flux, un filtre ou une longueur qu'il ne peut pas résoudre, des données compressées endommagées, un filtre inconnu, ou un filtre d'image (DCT, JPX, JBIG2, CCITT) placé ailleurs qu'en dernier sur une image. Les données d'image elles-mêmes ne sont pas mesurées, puisque l'extraction du texte ne les décode jamais. Un PDF chiffré ne peut pas être mesuré, car ses flux sont chiffrés : il est lu, borné seulement par la seconde ligne. En seconde ligne, le thread de travail est arrêté si le processus grossit de plus de 1 Go pendant sa lecture, après 20 s, ou quand l'appel se termine ; l'attente de son tour est comptée dans l'échéance de l'appel.
...
`web_search` interroge ses fournisseurs **dans l'ordre** : un fournisseur qui échoue, ou dont le débit est encore limité après un second essai, passe la main au suivant. La réponse dit quel fournisseur a répondu (`provider`) et pourquoi les précédents ne l'ont pas fait (`errors`).
...
| --- | --- | --- | --- |
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | aucune (le fournisseur par défaut) | pour certains résultats | La page HTML de DuckDuckGo, pas une API officielle. Titres, liens et extraits ; les publicités sont ignorées. Une page de captcha ou une page vide est une limitation de débit. 4 s entre deux recherches, à compter de la fin de la précédente. |
...
**Limitation de débit.** Un fournisseur qui oppose une limitation de débit à une recherche (HTTP 429, la page de captcha ou la page vide de DuckDuckGo) a droit à un essai de plus, après l'attente qu'il a demandée (`Retry-After`) ou `throttleWaitMs` (10 s), si l'échéance de l'appel en laisse le temps. Un fournisseur qui demande plus de 30 s n'est jamais retenté plus tôt qu'il ne l'a demandé : il est ignoré pendant tout ce temps (au moins `circuitBreaker.cooldownMs`), et quand aucun autre fournisseur ne répond, l'appel échoue aussitôt, `throttled`, avec l'attente demandée (`retryAfterMs`).

**Disjoncteur.** Un fournisseur dont le débit est encore limité après ce second essai est laissé de côté aussitôt, tout autre fournisseur après trois échecs de suite : pendant deux minutes, il est ignoré sans qu'aucune requête ne lui soit envoyée (`errors` dit jusqu'à quand). Puis il a droit à un nouvel essai. `circuitBreaker: { cooldownMs, failureThreshold }` change ces deux valeurs. Quand aucun fournisseur n'a répondu, l'erreur (`SearchUnavailableError`) dit s'ils étaient tous limités en débit (`throttled`) et quand un fournisseur ignoré aura droit à un nouvel essai (`retryAfterMs`) : une étude retente alors cette recherche une fois, plus tard.
...
| `callTimeoutMs` | `60000` | L'appel entier, quoi qu'il attende : robots.txt, l'espacement des requêtes, chaque redirection, le corps, et l'extraction de la page ou du PDF. Au-delà, tout est interrompu et l'appel échoue avec une `WebTimeoutError`. Cette échéance borne chaque tentative : avec `retry`, un appel peut durer jusqu'à `maxRetries + 1` fois cette durée, plus les attentes entre deux tentatives. |
...
| `hostIntervalMs` | `1000` | Temps minimal entre la fin d'une requête de `web_fetch` vers un hôte et le début de la suivante. |
| `throttleWaitMs` | `10000` | Combien de temps un fournisseur ou une source dont le débit est limité attend avant son second essai, quand il ne l'a pas dit (`Retry-After`). |
...
| `retry` | — | Nouvelles tentatives des appels en échec (`{ maxRetries }`) : erreurs serveur, dépassements de délai et défaillances réseau ; jamais un refus, une mise en place manquante (`WebConfigurationError`), une recherche à laquelle aucun fournisseur n'a répondu (`SearchUnavailableError`) ou une limitation de débit (HTTP 429, `SearchThrottledError`), à laquelle l'outil a déjà accordé son second essai. |
| `arxiv` | `{ baseUrl: 'https://export.arxiv.org', minIntervalMs: 3000 }` | L'API d'arXiv demande 3 s entre deux requêtes, une à la fois : ces 3 s se comptent à partir de la fin de la précédente. Un refus (HTTP 406, 429, 503) a droit à un essai de plus après une attente. |
...
- **Les caches et l'espacement des requêtes vivent en mémoire**, et sont perdus quand le processus se termine : les caches sont propres à chaque appel à `webTools()`, l'espacement est commun à tout le processus. Un autre processus sur la même machine est espacé à part.
