...
**Limitation de débit.** Un fournisseur qui oppose une limitation de débit à une recherche (HTTP 429, la page de captcha ou la page vide de DuckDuckGo) a droit à un essai de plus, après l'attente qu'il a demandée (`Retry-After`) ou `throttleWaitMs` (10 s), 30 s au plus, si l'échéance de l'appel en laisse le temps.
