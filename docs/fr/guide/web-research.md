...
- **Le texte des PDF** est lu avec le paquet facultatif [`unpdf`](https://github.com/unjs/unpdf) (Node.js 22 ou plus récent) : `npm install unpdf`. Sans lui, `web_fetch` le dit. Le titre et la date viennent du document. Les PDF sont lus un par un dans le processus, chacun dans un thread de travail. Avant que pdf.js ne lise un PDF, le thread de travail décompresse ses flux à travers leurs filtres (Flate, LZW et RunLength, y compris enchaînés, ainsi que les ASCII85 et ASCIIHex qui les enveloppent) et refuse le PDF au-delà de 256 Mo au total : cette mesure ne dépend en rien du reste du processus, si bien qu'un petit PDF qui se décompresse en gigaoctets est refusé même quand le thread principal est occupé. Un flux qu'il ne peut pas mesurer (chiffré, ou avec un filtre qu'il ne connaît pas, comme celui d'une image) est laissé à pdf.js. En seconde ligne, le thread de travail est arrêté si le processus grossit de plus de 1 Go pendant sa lecture, après 20 s, ou quand l'appel se termine ; l'attente de son tour est comptée dans l'échéance de l'appel.
