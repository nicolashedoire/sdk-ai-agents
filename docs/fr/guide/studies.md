...
Une recherche de l'existant qui a échoué parce que le service en a limité le débit — l'erreur de son outil dit `throttled`, comme le disent les outils Web du SDK — n'est pas perdue pour autant : elle est retentée une fois, après les autres recherches de l'existant et après l'attente demandée par les services (5 s au moins), quand l'exécution aurait encore une minute et des recherches devant elle. Une recherche dont le service a demandé plus de 150 s, ou plus de temps qu'il n'en reste à l'exécution, n'est pas retentée plus tôt qu'il ne l'a demandé. Le second essai est consigné comme une recherche à part entière (`retry: true`). Une recherche qui a échoué pour une autre raison n'est pas retentée, et une affirmation dont le second essai échoue aussi reste à vérifier, comme ci-dessus.
