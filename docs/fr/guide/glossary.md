...
| **Source d'outils** (*tool source*) | Une fonction qui construit des outils prêts à l'emploi à partir d'un système : `openApiTools`, `folderTools`, `databaseTools`, `webTools`, `governedAgentTool`, `cognitiveAgentTool`, et `connectMcpServer` pour les outils de n'importe quel serveur MCP. Servez-les en MCP en une ligne, ou donnez-les à vos propres agents. Voir [Outils](./tools), [Un serveur MCP pour tout](./mcp-recipes) et [Recherche sur le Web](./web-research). |
