...
Si el proveedor de respaldo es de otro fabricante, necesita su propia clave, en su `config` o en `providerConfig`: la clave del principal nunca se envía a otro fabricante. Recibe el modelo del agente solo si lo sirve; si no, usa su propio `defaultModel`, que es `gpt-5.4` para OpenAI y `claude-opus-5` para Anthropic si no se configura otro (Anthropic rechaza un nombre de modelo de OpenAI, y viceversa). El evento `intention.generated` indica qué proveedor respondió y con qué modelo.
...
Una ejecución que transmite su texto en streaming (`onText`) puede ver cómo falla una llamada al modelo después de que haya llegado parte de la respuesta. Un error que el fabricante envía en mitad del flujo cuenta como su equivalente HTTP: `server_error` (500) de OpenAI y `rate_limit_error` (429), `api_error` (500), `timeout_error` (504) y `overloaded_error` (529) de Anthropic se reintentan, los demás tipos no. Una conexión que se corta en mitad del flujo, un flujo que termina antes de que su respuesta esté completa y un flujo que no envía nada durante todo el tiempo límite del cliente (10 minutos por defecto; `timeout` en `providerConfig`, en el `config` de un proveedor de reserva o en las opciones de `OpenAIProvider` y `AnthropicProvider`) son fallos de conexión. Antes del reintento, o antes del proveedor de reserva, `onTextRestart` indica a quien llama que descarte el texto que había transmitido el intento fallido: el siguiente intento vuelve a escribir la respuesta (consulta [Recibir la respuesta en streaming](./governed-agents#_7-streaming-the-answer)).
