...
Una búsqueda de lo existente que falló porque el servicio le aplicó su límite de frecuencia — el error de su herramienta dice `throttled`, como dicen las herramientas web del SDK — no se pierde por eso: se intenta una vez más, después de las demás búsquedas de lo existente y tras la espera que pidieron los servicios (al menos 5 s, como mucho 150 s), cuando a la ejecución todavía le quedarían un minuto y búsquedas. El segundo intento se registra como una búsqueda propia (`retry: true`). Una búsqueda que falló por otro motivo no se vuelve a intentar, y una afirmación cuyo segundo intento también falla sigue por verificar, como arriba.
