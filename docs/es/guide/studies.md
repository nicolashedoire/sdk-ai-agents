...
Una búsqueda de lo existente que falló porque el servicio le aplicó su límite de frecuencia — el error de su herramienta dice `throttled`, como dicen las herramientas web del SDK — no se pierde por eso: se intenta una vez más, después de las demás búsquedas de lo existente y tras la espera que pidieron los servicios (al menos 5 s), cuando a la ejecución todavía le quedarían un minuto y búsquedas. Una búsqueda cuyo servicio pidió más de 150 s, o más tiempo del que le queda a la ejecución, no se vuelve a intentar antes de lo que pidió. El segundo intento se registra como una búsqueda propia (`retry: true`). Una búsqueda que falló por otro motivo no se vuelve a intentar, y una afirmación cuyo segundo intento también falla sigue por verificar, como arriba.
...
- **Ante la duda, bloquea.** Solo cuenta un veredicto con un `onObjective` verdadero o falso. Un elemento que se queda sin él sigue `unchecked`: permanece en el informe, señalado (aviso `uncheckedItems`), pero nunca llega a un prompt posterior, y la siguiente ejecución hace que el guardián lo juzgue primero. Un guardián que no juzga ninguno de los elementos de un pasaje hace fallar la ejecución. Cuando no se puede usar una reparación, o el proveedor falla en ella, se lee en su lugar la primera respuesta, con tolerancia: sus veredictos válidos cuentan, y los elementos que no tienen ninguno quedan sin juzgar (`usedAttempt: 1` en `study.model_called` cuando la reparación obtuvo respuesta). Una detención (un límite, una cancelación) sigue deteniendo la ejecución.
