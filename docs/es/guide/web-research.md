...
`web_search` consulta a sus proveedores **en orden**: un proveedor que falla, o que sigue alcanzando su límite de frecuencia tras un segundo intento, pasa el relevo al siguiente. La respuesta dice qué proveedor respondió (`provider`) y por qué no lo hicieron los anteriores (`errors`).
...
| --- | --- | --- | --- |
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | ninguna (el valor por defecto) | en algunos resultados | La página HTML de DuckDuckGo, no una API oficial. Títulos, enlaces y fragmentos; los anuncios se omiten. Una página de captcha o una página vacía cuenta como límite de frecuencia. 4 s entre búsquedas, contados desde el final de la anterior. |
...
**Límite de frecuencia.** Un proveedor que frena una búsqueda por límite de frecuencia (HTTP 429, la página de captcha o la página vacía de DuckDuckGo) recibe un intento más, tras la espera que pidió (`Retry-After`) o `throttleWaitMs` (10 s), cuando el plazo de la llamada deja tiempo para ello. Uno que pide más de 30 s nunca se vuelve a intentar antes de lo que pidió: se omite durante todo ese tiempo (al menos `circuitBreaker.cooldownMs`), y cuando ningún otro proveedor responde, la llamada falla de inmediato, `throttled`, con la espera pedida (`retryAfterMs`).

**Disyuntor (circuit breaker).** Un proveedor que sigue alcanzando su límite de frecuencia tras ese segundo intento se deja de lado de inmediato, y cualquier otro tras tres fallos seguidos: durante dos minutos, se omite sin enviarle ninguna solicitud (`errors` dice hasta cuándo). Después recibe un intento más. `circuitBreaker: { cooldownMs, failureThreshold }` cambia ambos valores. Cuando ningún proveedor respondió, el error (`SearchUnavailableError`) dice si todos habían alcanzado su límite de frecuencia (`throttled`) y cuándo se volverá a intentar con uno omitido (`retryAfterMs`): un estudio vuelve a intentar esa búsqueda una vez, más tarde.
...
| `hostIntervalMs` | `1000` | Tiempo mínimo entre el final de una solicitud de `web_fetch` a un host y el inicio de la siguiente. |
| `throttleWaitMs` | `10000` | Cuánto espera un proveedor o una fuente que alcanzó su límite de frecuencia antes de su segundo intento, cuando no lo indicó (`Retry-After`). |
...
| `retry` | — | Reintentos de las llamadas fallidas (`{ maxRetries }`): errores de servidor, tiempos límite agotados y fallos de red; nunca un rechazo, una configuración que falta (`WebConfigurationError`), una búsqueda que ningún proveedor respondió (`SearchUnavailableError`) ni un límite de frecuencia (HTTP 429, `SearchThrottledError`), al que la herramienta ya dio su segundo intento. |
| `arxiv` | `{ baseUrl: 'https://export.arxiv.org', minIntervalMs: 3000 }` | La API de arXiv pide 3 s entre solicitudes, de una en una: se cuentan desde el final de la anterior. Un rechazo (HTTP 406, 429, 503) recibe un intento más tras una espera. |
...
5. **Cortés.** `web_fetch` lee robots.txt ([RFC 9309](https://www.rfc-editor.org/rfc/rfc9309)) y nunca descarga lo que este prohíbe para `sdk-ai-agents`, redirecciones incluidas: se aplica el grupo que nombra `sdk-ai-agents`, si no `*`; gana la regla más larga, y `Allow` gana en caso de empate; se admiten los patrones `*` y `$`; los escapes de caracteres no reservados se decodifican antes de comparar (`%7E` es `~`). Un robots.txt que no existe (4xx) lo permite todo; uno que falla (5xx, 429) o al que no se puede llegar lo prohíbe todo. Leer robots.txt no retrasa la primera página; `Crawl-delay` espacia las solicitudes siguientes, y un retardo de más de 30 s hace rechazar la página siguiente hasta que se cumpla. Las solicitudes a cada host van de una en una, cada una tras un intervalo contado desde el final de la anterior (1 s para las páginas, 4 s para DuckDuckGo, 3 s para arXiv), y todos los `webTools()` del proceso comparten este ritmo; un límite de frecuencia recibe un intento más tras una espera, no una ráfaga de intentos; las respuestas se guardan en caché, y el agente de usuario dice quién pregunta. Las API de búsqueda no se rastrean: robots.txt no se aplica a ellas.
...
- **Las cachés y el ritmo entre solicitudes viven en memoria**, y se pierden cuando termina el proceso: las cachés son propias de cada llamada a `webTools()`, el ritmo es común a todo el proceso. Otro proceso en la misma máquina lleva su propio ritmo.
