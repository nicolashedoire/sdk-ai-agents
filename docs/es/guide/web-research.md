...
**Límite de frecuencia.** Un proveedor que frena una búsqueda por límite de frecuencia (HTTP 429, la página de captcha o la página vacía de DuckDuckGo) recibe un intento más, tras la espera que pidió (`Retry-After`) o `throttleWaitMs` (10 s), como mucho 30 s, cuando el plazo de la llamada deja tiempo para ello.
