...
| **Fuente de herramientas** (tool source) | Una función que construye herramientas listas para usar a partir de un sistema: `openApiTools`, `folderTools`, `databaseTools`, `cognitiveAgentTool`. Sírvelas por MCP con una línea, o dáselas a tus propios agentes. Consulta [Un servidor MCP para cualquier cosa](./mcp-recipes). |
