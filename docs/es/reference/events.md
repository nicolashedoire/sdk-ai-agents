...
| `study.passage_completed` | `passage`, `attempts` (2 cuando el guardián hizo que se rehiciera), `keptAttempt?` (1 cuando el primer intento era mejor que el segundo y se conservó), `items` (cada uno con su `collection`, `id`, `statement`, `status`, `sources`, `servesObjective`, los demás campos de una afirmación y sus campos propios), `reopenedBy?`, `reopen?` (`passage`, `focus`, `reason`: el pasaje anterior que pide reabrir; no está completo hasta que se ejecute de nuevo), `resumed?` (una ejecución lo reanudó solo para terminarlo) |
