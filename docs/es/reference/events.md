...
| `study.search` | `passage`, `purpose` (`research`, o `priorArt` para lo existente sobre las novedades), `tool`, `query`, `servesObjective`, `claims?` (las novedades que busca), `resultIds`, `results` (`id`, `title`, `locator`, `date?`), `error?` (la búsqueda falló), `throttled?` (falló por un límite de frecuencia), `retry?` (el segundo intento de una búsqueda de lo existente que alcanzó un límite de frecuencia), `skipped?` (`maxSearches`: no se ejecutó) |
