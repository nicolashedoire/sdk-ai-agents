...
| `cognition.thought` | `step`, `operation`, `patch`, `issues`, `failed`, `ignoredFields?`, `model?`, `requestedModel?`, `usage?` (`promptTokens`, `completionTokens`, `calls`, `unmeteredCalls?` — llamadas que no informaron de ningún número de tokens, `totalOnlyTokens?` — los totales que algunas de ellas informaron por sí solos) |
