...
| `webTools({ include?, prefix?, search?, circuitBreaker?, language?, userAgent?, timeoutMs?, maxResponseBytes?, maxRedirects?, hostIntervalMs?, robots?, allowPrivateNetwork?, lookup?, maxPdfBytes?, maxPdfPages?, cache?, retry?, arxiv?, wikipedia?, github? })` | `ToolDefinition[]` | `web_search`, `web_fetch`, `arxiv_search`, `wikipedia_search`, `github_search`, de solo lectura (`web_fetch` de riesgo medio, las demás bajo). Resultados de búsqueda `{ id, title, url, date?, excerpt, source }`; una página `{ url, finalUrl, title?, date?, language?, contentType, content, truncated, untrusted: true, hint? }`. Ninguna dirección fuera de Internet público salvo con `allowPrivateNetwork`; se respeta robots.txt. Consulta [Investigación web](../guide/web-research) |
...
| `normalizeUrl(url)` | `string \| undefined` | La URL por la que se reconoce un resultado de búsqueda: sin parámetros de seguimiento, sin fragmento y sin barra final; `undefined` para todo lo que no sea http(s) |
