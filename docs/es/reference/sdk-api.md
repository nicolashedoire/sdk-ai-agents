...
| `StudySearch` | `passage`, `purpose` (`research` o `priorArt`), `tool`, `query`, `servesObjective`, `claims?`, `resultIds`, `error?`, `throttled?` (falló por un límite de frecuencia), `retry?` (el segundo intento de una búsqueda de lo existente que alcanzó un límite de frecuencia), `skipped?` (`maxSearches`), `runId` |
...
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | `SearchProvider` | El proveedor por defecto de `web_search`, sin clave: la página HTML de DuckDuckGo, 4 s entre búsquedas, contados desde el final de la anterior |
...
`web_search` consulta a sus proveedores en orden; un proveedor que lanza un error pasa el relevo al siguiente. Uno que lanza `SearchThrottledError` (`retryAfterMs?`, la espera que pidió) o responde HTTP 429 recibe un intento más tras esa espera o `throttleWaitMs` (10 s), cuando el plazo de la llamada lo permite, y ninguno cuando pidió más de 30 s; si sigue limitado (o falla tres veces seguidas), se omite durante `circuitBreaker.cooldownMs` (2 minutos), o tanto tiempo como pidió si es más. Las solicitudes a un host van de una en una, cada una `minIntervalMs` después de que terminara la anterior, para todo el proceso (DuckDuckGo: 4 s por defecto). Las herramientas web lanzan `WebRequestRefusedError` para lo que rechazan a propósito (`reason`: `private-address`, `scheme`, `downgrade`, `redirects`, `robots`, `content-type`, `too-large`, `unreadable`, `pacing`), `WebHttpError` para una respuesta que no es 2xx (`status`), `WebTimeoutError` (una solicitud, el plazo de la llamada o el presupuesto de tiempo de una extracción), `WebConfigurationError` para una configuración que falta (`unpdf`, un token de GitHub) y `SearchUnavailableError` cuando ningún proveedor respondió (`failures`; `throttled` cuando todos habían alcanzado su límite de frecuencia, `retryAfterMs?` cuando se volverá a intentar con uno omitido). `retry` nunca reintenta un rechazo, una configuración que falta, una búsqueda que ningún proveedor respondió ni un límite de frecuencia (HTTP 429, `SearchThrottledError`).
