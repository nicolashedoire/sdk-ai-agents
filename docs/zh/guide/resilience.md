...
流式输出文本的运行（`onText`）可能会遇到模型调用在部分回答已经到达之后才失败的情况。厂商在流的中途发送的错误按其对应的 HTTP 错误处理：OpenAI 的 `server_error`（500）以及 Anthropic 的 `rate_limit_error`（429）、`api_error`（500）、`timeout_error`（504）和 `overloaded_error`（529）会被重试，其他类型则不会。在流的中途断开的连接、在回答完整之前就结束的流，以及在客户端的超时时间（默认 10 分钟；即 `providerConfig`、备用提供商的 `config` 或 `OpenAIProvider` 和 `AnthropicProvider` 的选项中的 `timeout`）内没有发送任何内容的流，都属于连接失败。在重试之前，或在切换到备用提供商之前，`onTextRestart` 会告诉调用方丢弃失败的那次尝试已经流式输出的文本：下一次尝试会重新写出回答（参见[流式输出回答](./governed-agents#_7-streaming-the-answer)）。
