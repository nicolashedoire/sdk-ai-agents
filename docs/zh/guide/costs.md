...
预算统计的是记录在运行中的模型调用，与 `getRunCost` 相同：受治理智能体的推理步骤；认知智能体的思维（包括修复和失败的尝试）、工具选择和类型化决策；以及用 `sdk.decisions` 做出的类型化决策。带 `agentId` 的上限统计该智能体的模型调用——以及用 `agentId` 指明它的 `sdk.decisions` 调用；不带 `agentId` 的上限统计全部调用，包括不指明智能体做出的类型化决策。预算从不拒绝 `sdk.decisions` 的调用：它拒绝的是工具调用和认知智能体的步骤。
