...
- 每个模块都有单元测试
- 使用 Vitest
- 没有任何测试会调用真实的付费服务。较新模块的测试套件不使用模块 mock：SDK 的各端口由 `src/__tests__/support/` 中的测试替身实现，HTTP 适配器则针对本地服务器运行
- 较早的测试套件（SDK 入口、OpenAI 和 Anthropic 提供商、推理引擎、回退、回归测试运行器）仍然用 `vi.mock` 替换厂商库 `openai` 和 `@anthropic-ai/sdk`，并用 `vi.fn()` 替换部分依赖，例如事件存储

### 集成测试 {#integration-tests}

- 针对完整工作流程的集成测试
- LLM 提供商由经过模拟的厂商库（`vi.mock`）替代；没有任何测试会调用真实的 API
- 使用不同事件存储的测试
...
1. 检查 `src/__tests__/support/` 中的测试替身，或较早测试套件中的模拟对象，是否仍与它们所替代的接口一致
2. 检查依赖是否是最新的
3. 以监听模式运行测试，实时查看错误
