...
| **Fonte de ferramentas** (tool source) | Uma função que constrói ferramentas prontas a partir de um sistema: `openApiTools`, `folderTools`, `databaseTools`, `webTools`, `governedAgentTool`, `cognitiveAgentTool`, e `connectMcpServer` para as ferramentas de qualquer servidor MCP. Sirva-as por MCP com uma linha, ou entregue-as aos seus próprios agentes. Veja [Ferramentas](./tools), [Um servidor MCP para qualquer coisa](./mcp-recipes) e [Pesquisa na Web](./web-research). |
