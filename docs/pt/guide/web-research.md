...
`web_search` consulta os seus provedores **em ordem**: um provedor que falha, ou que ainda atinge um limite de requisições depois de uma segunda tentativa, passa a vez ao seguinte. A resposta diz qual provedor respondeu (`provider`) e por que os anteriores não responderam (`errors`).
...
| --- | --- | --- | --- |
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | nenhuma (o padrão) | alguns resultados | A página HTML do DuckDuckGo, não uma API oficial. Títulos, links e trechos; os anúncios são ignorados. Uma página de captcha ou uma página vazia conta como limite de requisições. 4 s entre pesquisas, a partir do fim da anterior. |
...
| `hostIntervalMs` | `1000` | Tempo mínimo entre o fim de uma requisição de `web_fetch` a um host e o início da seguinte. |
| `throttleWaitMs` | `10000` | Quanto tempo um provedor ou uma fonte que atingiu um limite de requisições espera antes da segunda tentativa, quando não disse quanto (`Retry-After`). |
...
5. **Educado.** `web_fetch` lê o robots.txt ([RFC 9309](https://www.rfc-editor.org/rfc/rfc9309)) e nunca busca o que ele proíbe para `sdk-ai-agents`, redirecionamentos incluídos: o grupo que nomeia `sdk-ai-agents`, senão `*`; a regra mais longa vence, `Allow` vence em caso de empate; padrões `*` e `$`; os escapes de caracteres não reservados são decodificados antes da comparação (`%7E` é `~`). Um robots.txt ausente (4xx) permite tudo; um que falha (5xx, 429) ou que não pode ser acessado proíbe tudo. Ler o robots.txt não atrasa a primeira página; `Crawl-delay` espaça as requisições seguintes, e um intervalo maior que 30 s recusa a próxima página até lá. As requisições a cada host vão uma de cada vez, cada uma depois de um intervalo contado a partir do fim da anterior (1 s para páginas, 4 s para o DuckDuckGo, 3 s para o arXiv), e todos os `webTools()` do processo compartilham esse espaçamento; um limite de requisições recebe mais uma tentativa depois de uma espera, não uma rajada delas; as respostas ficam em cache, e o user agent diz quem está pedindo. As APIs de pesquisa não são rastreadas: o robots.txt não se aplica a elas.
...
- **Os caches e o espaçamento vivem em memória**, e se perdem quando o processo termina: os caches por chamada de `webTools()`, o espaçamento para o processo inteiro. Outro processo na mesma máquina tem o seu próprio espaçamento.
