...
Uma execução que faz streaming do seu texto (`onText`) pode ver uma chamada ao modelo falhar depois que parte da resposta chegou. Um erro que o fornecedor envia no meio do stream conta como o seu equivalente HTTP: o `server_error` (500) da OpenAI e os `rate_limit_error` (429), `api_error` (500), `timeout_error` (504) e `overloaded_error` (529) da Anthropic geram novas tentativas, os outros tipos não. Uma conexão que cai no meio do stream, um stream que termina antes de a sua resposta estar completa e um stream que não envia nada durante o timeout do cliente (10 minutos por padrão; `timeout` em `providerConfig`, no `config` de um fallback ou nas opções de `OpenAIProvider` e `AnthropicProvider`) são falhas de conexão. Antes da nova tentativa, ou antes do provedor de fallback, `onTextRestart` avisa quem chamou para descartar o texto que a tentativa que falhou tinha enviado em streaming: a próxima tentativa escreve a resposta de novo (veja [Streaming da resposta](./governed-agents#_7-streaming-the-answer)).
