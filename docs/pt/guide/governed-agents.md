...
- Com `onText`, os provedores OpenAI e Anthropic integrados chamam a API de streaming do seu fornecedor, e o resultado e os eventos da execução continuam os mesmos que sem essa opção. Os custos também, exceto com um servidor compatível com a OpenAI (uma `baseURL`): o consumo de uma resposta enviada em streaming só é pedido na própria API da OpenAI, a menos que `providerConfig.openai.includeStreamUsage` esteja definido, e uma chamada sem consumo informado conta como não medida nos orçamentos.
