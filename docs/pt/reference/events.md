...
| `cognition.thought` | `step`, `operation`, `patch`, `issues`, `failed`, `ignoredFields?`, `model?`, `requestedModel?`, `usage?` (`promptTokens`, `completionTokens`, `calls`, `unmeteredCalls?` — chamadas que não informaram nenhuma contagem de tokens, `totalOnlyTokens?` — os totais que algumas delas informaram sozinhos) |
