...
| `StudySearch` | `passage`, `purpose` (`research` ou `priorArt`), `tool`, `query`, `servesObjective`, `claims?`, `resultIds`, `error?`, `throttled?` (falhou por um limite de requisições), `retry?` (a segunda tentativa de uma pesquisa do estado da técnica que atingiu um limite de requisições), `skipped?` (`maxSearches`), `runId` |
...
Cada uma devolve `ToolDefinition`s prontas: passe-as por `sdk.defineTool` antes de entregá-las a um agente (os agentes recebem `Tool[]`), ou diretamente para o `tools` de um servidor MCP. Veja [Ferramentas](../guide/tools) e [Um servidor MCP para qualquer coisa](../guide/mcp-recipes).
...
| `duckDuckGo({ region?, minIntervalMs?, baseUrl? })` | `SearchProvider` | O provedor padrão de `web_search`, sem chave: a página HTML do DuckDuckGo, 4 s entre pesquisas, contados a partir do fim da anterior |
...
`web_search` consulta os seus provedores em ordem; um provedor que lança uma exceção passa a vez ao seguinte. Um que lança `SearchThrottledError` (`retryAfterMs?`, a espera que ele pediu) ou responde HTTP 429 recebe mais uma tentativa depois dessa espera ou de `throttleWaitMs` (10 s), quando o prazo da chamada deixa tempo, e nenhuma quando pediu mais de 30 s; ainda limitado (ou falhando três vezes seguidas), ele é pulado durante `circuitBreaker.cooldownMs` (2 minutos), ou pelo tempo que pediu, se for maior. As requisições a um host vão uma de cada vez, cada uma `minIntervalMs` depois do fim da anterior, para o processo inteiro (DuckDuckGo: 4 s por padrão). As ferramentas Web lançam `WebRequestRefusedError` para o que recusam de propósito (`reason`: `private-address`, `scheme`, `downgrade`, `redirects`, `robots`, `content-type`, `too-large`, `unreadable`, `pacing`), `WebHttpError` para uma resposta fora de 2xx (`status`), `WebTimeoutError` (uma requisição, o prazo da chamada ou o orçamento de tempo de uma extração), `WebConfigurationError` para uma configuração ausente (`unpdf`, um token do GitHub) e `SearchUnavailableError` quando nenhum provedor respondeu (`failures`; `throttled` quando todos estavam limitados, `retryAfterMs?` quando um provedor pulado será tentado de novo). `retry` nunca tenta de novo uma recusa, uma configuração ausente, uma pesquisa à qual nenhum provedor respondeu ou um limite de requisições (HTTP 429, `SearchThrottledError`).
