...
| `retry` | `Partial<RetryPolicy> \| false` | Política de novas tentativas do LLM, por provedor, antes do fallback. Também é o padrão das novas tentativas do cliente Jev (`jev.maxRetries` e `jev.retryBaseDelayMs` o substituem). Aplicada a um `llmProvider` injetado só quando definida explicitamente, e nunca a um `FallbackProvider` passado como `llmProvider` nem aos provedores dele |
