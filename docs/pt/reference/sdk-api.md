...
As execuções são comparadas **pelo que os seus eventos significam**, nunca pelo id dos eventos (cada execução tem ids novos). Os eventos são emparelhados em ordem pelo tipo e pelo assunto — a ferramenta, a política, a operação, a resposta — e depois os seus dados são comparados. Nunca são comparados: os ids dos eventos, os horários, os metadados e os campos de dados `agentId`, `approvalId`, `delayMs`, `duration`, `durationMs`, `elapsedMs`, `eventId`, `observedAt`, `recordedAt`, `replayOf`, `sourceEventId` e `usage`. Uma execução que volta a fazer a mesma coisa passa; uma ferramenta chamada com outros argumentos é apontada onde a chamada aconteceu (`parameters.metric: "churn" → "revenue"`); um `action.executed` que virou `action.failed` é uma única mudança, não uma perda mais um acréscimo.
...
| `compareStructureOnly` | Validação | As diferenças de dados dão `partial`, não `fail` |
...
| `exportTestResults(results, 'junit' \| 'json' \| 'json-summary', { outputPath?, includeDetails? })` | `Promise<string>` | JUnit XML: um `<testsuite>` por suíte, um tempo esgotado conta como erro |

Os ids dos agentes são novos em cada processo: uma suíte também registra o **nome** do seu agente, e outro processo a executa com o seu agente desse nome (primeiro com o id de agente da suíte, quando esse agente está no SDK). Quando dois agentes de um mesmo SDK compartilham o nome, informe o id. As suítes salvas por versões anteriores não têm nome: só rodam no processo que as criou. Opções: `parallel` (os testes de uma suíte ao mesmo tempo), `stopOnFirstFailure` (só em execuções sequenciais), `filterTags`, `excludeTags`, `timeout` (ms por teste, 60 000 por padrão; passado esse tempo, a execução é cancelada e o teste fica `timeout`) e `detection` (as opções de regressão acima).
...
| `compareVersions(agent, version1, version2, options?)` | `Promise<ImpactAnalysis>` | `analyzeImpact` sobre as execuções de um agente governado (o seu nome, ou o id de um agente deste SDK) registradas com cada versão: a sua `version` ou o seu `configHash`. Os replays ficam de fora; uma versão desconhecida lança um erro que lista as registradas |

Os eventos do ciclo de vida das execuções de um agente governado (`run.started`, `run.completed`…) registram o seu `agentName`, o seu `agentVersion` e o seu `configHash`: dois agentes com o mesmo nome são um mesmo agente em duas versões, ou em dois processos. As execuções registradas por versões anteriores só têm o id e a versão.
...
| `subscribe(listener, { runId?, agentId?, types?, maxQueued? })` | `() => void`: todos os eventos de todas as execuções que correspondem ao filtro (`agentId` é `metadata.agentId`), até você chamar a função devolvida, que descarta os eventos ainda não entregues |
