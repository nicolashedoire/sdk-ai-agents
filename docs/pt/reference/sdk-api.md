...
| `lines` | Uma por modelo, modelo pedido e origem: chamadas, tokens das chamadas que os informaram, `unmeteredCalls` quando houver, `totalOnlyTokens` para os totais que algumas informaram sozinhos, `costUsd` quando o modelo tem preço e alguma chamada da linha informou os tokens; `model` é `(unknown)` para uma chamada que não registrou nenhum nome de modelo |
