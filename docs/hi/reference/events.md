...
| `policy.checked` / `policy.violated` | `intention`, `validation` / `reason`, `violatedPolicies` (`allowed-tools` जब कॉल करने वाले ने ऐसा टूल इस्तेमाल किया जो उसे नहीं दिया गया था; बजट नीति का id जब उसका कॉल-बजट खत्म हो चुका हो), और `step` जब किसी बजट या timeout नीति ने संज्ञानात्मक run का कोई कदम ठुकराया हो (तब `intention` होता है `{ type: 'continue' }`) |
