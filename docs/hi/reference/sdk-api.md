...
`web_search` अपने प्रदाताओं से क्रम से पूछता है; जो प्रदाता error फेंके, वह अगले को काम सौंप देता है। जो `SearchThrottledError` फेंके (`retryAfterMs?`, उसका माँगा इंतज़ार) या HTTP 429 से जवाब दे, उसे उस इंतज़ार या `throttleWaitMs` (10 s) के बाद, अधिकतम 30 s, एक और प्रयास मिलता है, जब कॉल की समय-सीमा में इसके लिए जगह हो; फिर भी throttling लगी रहे (या वह लगातार तीन बार विफल हो), तो उसे `circuitBreaker.cooldownMs` (2 मिनट) तक छोड़ दिया जाता है। किसी host पर अनुरोध पूरे प्रोसेस में एक-एक करके जाते हैं, हर एक पिछले के खत्म होने के `minIntervalMs` बाद (DuckDuckGo: डिफ़ॉल्ट रूप से 4 s)। जो चीज़ें वेब टूल जान-बूझकर ठुकराते हैं, उनके लिए वे `WebRequestRefusedError` फेंकते हैं (`reason`: `private-address`, `scheme`, `downgrade`, `redirects`, `robots`, `content-type`, `too-large`, `unreadable`, `pacing`), गैर-2xx उत्तर के लिए `WebHttpError` (`status`), `WebTimeoutError` (किसी अनुरोध, कॉल की समय-सीमा, या सामग्री निकालने के समय-बजट के लिए), अधूरे सेटअप (`unpdf`, GitHub token) के लिए `WebConfigurationError` और जब किसी प्रदाता ने जवाब नहीं दिया तो `SearchUnavailableError` (`failures`; सब पर throttling लगी हो तो `throttled`, और छोड़े गए प्रदाता को फिर कब आज़माया जाएगा, यह `retryAfterMs?`)। `retry` किसी अस्वीकृति, अधूरे सेटअप या ऐसी खोज को कभी दोबारा नहीं आज़माता जिसका किसी प्रदाता ने जवाब नहीं दिया।
