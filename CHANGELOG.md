...
- **The built-in providers are exported.** `OpenAIProvider`, `AnthropicProvider` (and `DEFAULT_ANTHROPIC_MODEL`) and `FallbackProvider` (and its `FallbackResult` type) can be imported from the package, so a fallback chain can be composed by hand and given as `llmProvider`: its failovers are recorded as `provider.fallback` events, as with `fallbackProviders`. `retry` does not apply to the providers of such a chain: wrap one in `RetryingLLMProvider` and give its client `maxRetries: 0` to retry it before failing over (those retries are not recorded as `provider.retry` events). `VendorClientOptions` (`maxRetries`, `baseURL`) is exported with them.
