...
- **Cancellation reaches the vendor.** An already cancelled call is no longer sent (nor billed), and an aborted call cuts the HTTP request instead of waiting for the full answer. The reasoning engine also honours `ReasoningContext.abortSignal`.
...
- **Recent Claude models work.** Claude Opus 4.7 and later, Sonnet 5 and the Fable and Mythos models refuse `temperature` with a 400, which the SDK always sent: it is no longer sent to them. Without a `maxTokens` setting, Claude 4 and later get 16 000 output tokens (their thinking counts in it), Opus 4 and 4.1 8 192, Claude 3 4 096.
...
### Added
- **Tool results in each vendor's native format.** The OpenAI and Anthropic providers get the assistant's tool call and a tool result tied to it by id (`tool` message, `tool_result` block) instead of plain-text lines. A turn Claude returned is sent back unchanged, thinking blocks included, and Claude is asked for one tool call per turn, as the SDK runs one tool per step. A custom `llmProvider` keeps the plain-text lines unless it declares `nativeToolMessages`.
