...
### Changed
- `metadata.requiresApproval: true` on a tool is now **enforced**: every call waits for `sdk.approveAction` (policy id `tool-requires-approval`) even when every policy allows it. It was only descriptive before, so a tool marked this way ran without any approval. A replay does not ask again for these tool-level approvals (policy approvals still apply).
- Nobody can approve a call whose caller has left. A pending approval is cancelled — the action refused, recorded as `approval.rejected` with a `reason` — when the caller's `signal` aborts (a stopped run, an MCP client that cancelled, a stdio client that exited, an HTTP connection that closed) or after `approvalTimeoutMs` without a decision (`executeTool` option; **50 000 ms by default for MCP servers**, the only limit for a stateless HTTP server). The call is checked again after the approval, before the tool runs. A cancelled approval fails the call with a `PolicyViolationError` instead of a raw error.
...
- The file event store's flush timer no longer keeps the process alive; pending events are written just before the process exits. A script or a stdio MCP server can now end without `process.exit()`.
...
- **An MCP server for anything, in one line.** New tool sources in the core package, usable by agents and by MCP servers alike:
  ...
  - `databaseTools({ database })` (`list_tables`, `describe_table`, `query`) with `sqliteReadOnly(db)` (`node:sqlite` or better-sqlite3: `PRAGMA query_only`, writing statements refused, rows read and converted one at a time) and `postgresReadOnly({ pool } | { client })` (`BEGIN READ ONLY` verified with `SHOW transaction_read_only`, `SET LOCAL statement_timeout`, `ROLLBACK` then `pg_advisory_unlock_all()`, one statement enforced by the server, transactions never interleaved on one client, a pool refused as a client; only SQL errors — by SQLSTATE — are shown to the model). A lexical check (`assertSingleQuery`, SQLite and PostgreSQL syntax) accepts one `SELECT`/`WITH`/`VALUES` statement without bind parameters and with balanced parentheses; rows are capped (`maxRows`, fetched as limit + 1 to report truncation) and made JSON-safe (arrays keep 100 items).
