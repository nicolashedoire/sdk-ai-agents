...
### Changed
- **Policies are checked when they are applied.** `createSDK({ defaultPolicies })`, `defineGlobalPolicy`, an agent's `policies` (`createAgent`, `createCognitiveAgent`) and `agent.setPolicy` throw a `ValidationError` naming the field (`Validation failed: policy 'caps' rules[0].metadata.budgetLimit.maxToolCalls - must be a finite number >= 0, got NaN`) before anything is registered, when a limit cannot be checked: a `maxSteps`, `maxTokens` or `maxDuration` `value` that is not a finite number above 0 (`NaN`, `0` or a missing value turned the limit off; a numeric string such as `'10'` worked only by coercion and is now refused); a limit in a policy type that never reads it (`maxDuration` outside a `timeout` policy, `maxSteps`, `maxTokens` or `budgetLimit` outside a `budget` one), which did nothing; a `budgetLimit` without a known `period`, without any cap, with a non-string `agentId` or `toolName`, or with a cap that is not a finite number ≥ 0 (`NaN` turned it off, `null` acted as 0, a string such as `'2'` lost the protection against concurrent calls); `rules` that are not a list of objects. Disabled policies are not checked, and the rules of `custom` and `allowlist` policies are not read as limits. A limit changed after its policy was applied, and no longer checkable, refuses the calls it covers. **Migration:** convert values read from the environment (`Number(process.env.MAX_STEPS)`), and remove a rule or set `enabled: false` instead of a value of 0.
