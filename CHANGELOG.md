...
- **The built-in providers are exported.** `OpenAIProvider`, `AnthropicProvider` (and `DEFAULT_ANTHROPIC_MODEL`) and `FallbackProvider` (and its `FallbackResult` type) can be imported from the package, so a fallback chain can be composed by hand and given as `llmProvider`: its failovers are recorded as `provider.fallback` events, as with `fallbackProviders`. `VendorClientOptions` (`maxRetries`, `baseURL`) is exported with them.
