...
- **`maxCost` budgets apply.** `BudgetLimit.maxCost` was never read: a cost cap never refused anything. Each model call of a governed agent now adds its cost (from `SDKConfig.pricing`, on the model that answered) to the agent's budgets per period, and a `budgetLimit` with `maxCost` refuses tool calls once it is exceeded. When a model has no price, the cap cannot be checked and tool calls are refused with that reason (prices are never guessed). `BudgetUsage` has `costUsd` and `unpricedCalls`; `PolicyEngine.recordModelUsage` and `BudgetTracker.recordModelUsage` record a model call.
