...
- Tests: no suite mocks a module or a function any more. The OpenAI and Anthropic adapters, the SDK and the reasoning engine run against a local server answering in the vendor's format, event stores are real, and `no-mocks.test.ts` refuses `vi.mock`, `vi.fn` and `vi.spyOn` (639 → 675 tests).
