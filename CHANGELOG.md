...
### Added
- **`baseURL` for the built-in providers.** `providerConfig.openai.baseURL`, `providerConfig.anthropic.baseURL` and `fallbackProviders[].config.baseURL` point the OpenAI or Anthropic provider at a compatible endpoint (Azure OpenAI, a local model server, a gateway) or a proxy.
