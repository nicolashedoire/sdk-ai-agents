...
- `EventType` says, in its JSDoc, which three types the SDK never records: `intention.rejected`, `tool.failed` and `error.occurred`, kept for events appended by your own code. The architecture page's copy of the type was stale (14 of its 33 types missing) and now matches the source.
