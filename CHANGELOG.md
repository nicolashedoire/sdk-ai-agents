...
- **SQL event stores wait for their schema.** `SQLEventStore` (and `PostgreSQLEventStore`, `SQLiteEventStore`) created its table without waiting: on a new database the first write could run before the table existed, and a database unreachable at start-up left an unhandled rejection that stops a Node process. Every query now waits for the schema, and a schema failure is reported by the first operation. The table name, written into the SQL, must be an identifier (`events`, optionally `app.events`); anything else is refused with a `ValidationError`. For subclasses: `initializeSchema` receives the connection to use.
